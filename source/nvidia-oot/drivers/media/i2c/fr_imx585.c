 ...
 *
 * fr_imx585.c - Framos fr_imx585.c driver
 */
...
#define IMX585_HIGH_GAIN_REG_MIN 34

#define IMX585_MAX_BLACK_LEVEL_10BPP 1023
#define IMX585_MAX_BLACK_LEVEL_12BPP 4095
#define IMX585_DEFAULT_BLACK_LEVEL_10BPP 50
#define IMX585_DEFAULT_BLACK_LEVEL_12BPP 200

#define IMX585_MIN_SHR0_CLEAR_HDR_LENGTH 10
#define IMX585_MIN_SHR0_LENGTH 8
#define IMX585_MIN_INTEGRATION_LINES 2

#define IMX585_MAX_GAIN_CLEAR_HDR 240
#define IMX585_MAX_GAIN 720
...
const char * const imx585_data_rate_menu[] = {
	[IMX585_2376_MBPS] = "2376 Mbps/lane",
	[IMX585_2079_MBPS] = "2079 Mbps/lane",
	[IMX585_1782_MBPS] = "1782 Mbps/lane",
	[IMX585_1440_MBPS] = "1440 Mbps/lane",
	[IMX585_1188_MBPS] = "1188 Mbps/lane",
	[IMX585_891_MBPS] = "891 Mbps/lane",
	[IMX585_720_MBPS] = "720 Mbps/lane",
	[IMX585_594_MBPS] = "594 Mbps/lane",
};
...
static const char * const imx585_adc_mode_menu[] = {
	[0] = "Normal mode",
	[1] = "Dual ADC w combination",
};

static const char * const imx585_gain_offset_menu[] = {
	[0] = "0dB",
	[1] = "+6dB",
	[2] = "+12dB",
	[3] = "+18dB",
...
static const char * const imx585_gc_gains[] = {
	[0] = "1/1",
	[1] = "1/2",
	[2] = "1/4",
	[3] = "1/8",
...
static struct v4l2_ctrl_config imx585_custom_ctrl_list[] = {
	{
		...
		.id = TEGRA_CAMERA_CID_ADC_MODE,
		.name = "ADC mode",
		.type = V4L2_CTRL_TYPE_MENU,
		.min = 0,
		.max = ARRAY_SIZE(imx585_adc_mode_menu) - 1,
		.def = 0,
		.qmenu = imx585_adc_mode_menu,
	},
	{
		.ops = &imx585_custom_ctrl_ops,
		.id = TEGRA_CAMERA_CID_GAIN_OFFSET,
		.name = "Gain offset",
		.type = V4L2_CTRL_TYPE_MENU,
		.min = 0,
		.max = ARRAY_SIZE(imx585_gain_offset_menu) - 1,
		.def = 1,
		.qmenu = imx585_gain_offset_menu,
	},
	{
		.ops = &imx585_custom_ctrl_ops,
		.id = TEGRA_CAMERA_CID_EXPOSURE_THRESHOLD,
		.name = "Exposure threshold low",
	...
	{
		.ops = &imx585_custom_ctrl_ops,
		.id = TEGRA_CAMERA_CID_EXPOSURE_THRESHOLD_HIGH,
		.name = "Exposure threshold high",
		.type = V4L2_CTRL_TYPE_INTEGER64,
		.flags = V4L2_CTRL_FLAG_SLIDER,
		.min = 0,
		.max = 0x1FFF,
	...
	{
		.ops = &imx585_custom_ctrl_ops,
		.id = TEGRA_CAMERA_CID_GC_START_POINT_FIRST,
		.name = "GC start point 1",
		.type = V4L2_CTRL_TYPE_INTEGER64,
		.flags = V4L2_CTRL_FLAG_SLIDER,
		.min = 0,
		.max = 0x1FFFF,
	...
	{
		.ops = &imx585_custom_ctrl_ops,
		.id = TEGRA_CAMERA_CID_GC_START_POINT_SECOND,
		.name = "GC start point 2",
		.type = V4L2_CTRL_TYPE_INTEGER64,
		.flags = V4L2_CTRL_FLAG_SLIDER,
		.min = 0,
		.max = 0x1FFFF,
	...
	{
		.ops = &imx585_custom_ctrl_ops,
		.id = TEGRA_CAMERA_CID_GC_GAIN_POINT_FIRST,
		.name = "GC gain point 1",
		.type = V4L2_CTRL_TYPE_MENU,
		.min = 0,
		.max = ARRAY_SIZE(imx585_gc_gains) - 1,
		.def = 4,
		.qmenu = imx585_gc_gains,
	},
	{
		.ops = &imx585_custom_ctrl_ops,
		.id = TEGRA_CAMERA_CID_GC_GAIN_POINT_SECOND,
		.name = "GC gain point 2",
		.type = V4L2_CTRL_TYPE_MENU,
		.min = 0,
		.max = ARRAY_SIZE(imx585_gc_gains) - 1,
		.def = 6,
...
static int imx585_write_reg(struct camera_common_data *s_data,
				u16 addr, u8 val)
{
	int err;
...
static int imx585_write_reg_broadcast(struct camera_common_data *s_data,
							u16 addr, u8 val)
{
	int err;
	struct device *dev = s_data->dev;
...
static int imx585_broadcast_buffered_reg(struct camera_common_data *s_data,
				u16 addr_low, u8 number_of_registers, u32 val)
{
	...
	if (!s_data->group_hold_active) {
		err = imx585_write_reg_broadcast(s_data, REGHOLD, 0x01);
		if (err) {
			dev_err(dev, "%s: GRP_PARAM_HOLD error\n", __func__);
			return err;
		}
	...
	for (i = 0; i < number_of_registers; i++) {
		err = imx585_write_reg_broadcast(s_data, addr_low + i,
					(u8)(val >> (i * 8)));
		if (err) {
			dev_err(dev, "%s: BUFFERED register write error\n",
					__func__);
	...
	if (!s_data->group_hold_active) {
		err = imx585_write_reg_broadcast(s_data, REGHOLD, 0x00);
		if (err) {
			dev_err(dev, "%s: GRP_PARAM_HOLD erroror\n", __func__);
			return err;
		}
...
static int imx585_update_ctrl(struct tegracam_device *tc_dev, int ctrl_id,
				...
				u64 min_val, u64 max_val)
{
	...
	if (ctrl) {
		...
		case TEGRA_CAMERA_CID_TEST_PATTERN:
			ctrl->qmenu = imx585_test_pattern_menu;
			ctrl->maximum = max_val;
			break;
		case TEGRA_CAMERA_CID_DATA_RATE:
			ctrl->qmenu = imx585_data_rate_menu;
			ctrl->maximum = max_val;
			break;
		}
	}
...
static int imx585_set_black_level(struct tegracam_device *tc_dev, s64 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;
	...
	err = imx585_write_buffered_reg(s_data, BLKLEVEL_LOW, 2, black_level_reg);
	if (err) {
...
static int imx585_set_gain(struct tegracam_device *tc_dev, s64 val)
{
	...
	if (val > ctrlprops->max_gain_val) {
		val = ctrlprops->max_gain_val;
	...
	if (priv->broadcast_ctrl == BROADCAST)
		err = imx585_broadcast_buffered_reg(s_data, GAIN_LOW, 2, gain);
	else
...
static int imx585_set_conversion_gain(struct tegracam_device *tc_dev, bool val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	const struct sensor_mode_properties *mode =
	...
	err = imx585_write_reg(s_data, FDG_SEL0, val);
	if (err) {
	...
	if (val) {
		err = imx585_read_buffered_reg(s_data, GAIN_LOW, 2, &curr_gain);
		if (err) {
			dev_err(dev, "%s: reading gain value error\n", __func__);
			return err;
		}
		min_high_gain = IMX585_HIGH_GAIN_REG_MIN * IMX585_MAX_GAIN_DB
			* mode->control_properties.gain_factor / IMX585_MAX_GAIN_DEC;

		if (curr_gain < IMX585_HIGH_GAIN_REG_MIN) {
			dev_warn(dev, "%s:Gain value too large for high conversion gain\n", __func__);
			curr_gain = IMX585_HIGH_GAIN_REG_MIN * IMX585_MAX_GAIN_DB
				* mode->control_properties.gain_factor
				/ IMX585_MAX_GAIN_DEC;
			err = imx585_set_gain(tc_dev, curr_gain);
			if (err) {
...
static int imx585_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
	...
	exposure_min_range = IMX585_MIN_INTEGRATION_LINES
					* priv->line_time / IMX585_K_FACTOR;

	exposure_max_range = (priv->frame_length - IMX585_MIN_INTEGRATION_LINES)
					* priv->line_time / IMX585_K_FACTOR;
	fr_update_ctrl_range(tc_dev, TEGRA_CAMERA_CID_EXPOSURE,
				 exposure_min_range, exposure_max_range);

	if (priv->broadcast_ctrl == BROADCAST)
		err = imx585_broadcast_buffered_reg(s_data,
...
static int imx585_set_compression(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err = 0;

	if (fr_get_v4l2_ctrl_value(tc_dev, TEGRA_CAMERA_CID_ADC_MODE, CURRENT) &&
			(s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB12_1X12
			|| s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB10_1X10)) {
		err = imx585_write_reg(s_data, CCMP_EN, 1);
	} else {
		err = imx585_write_reg(s_data, CCMP_EN, 0);
	}

	if (err) {
		dev_err(dev, "%s: failed to set compression\n", __func__);
		return err;
	}
...
static int imx585_set_gc_point_first(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;

	err = imx585_write_buffered_reg(s_data, CCMP1_EXP_LOW, 3, val);
	if (err) {
		dev_err(dev, "%s: failed to set first compression point\n", __func__);
		return err;
	}
...
static int imx585_set_gc_point_second(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;

	err = imx585_write_buffered_reg(s_data, CCMP2_EXP_LOW, 3, val);
	if (err) {
		dev_err(dev, "%s: failed to set second compression point\n", __func__);
		return err;
	}
...
static int imx585_set_gc_gain_point_first(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;

	err = imx585_write_reg(s_data, ACMP1_EXP, (u8)val);
	if (err) {
		dev_err(dev, "%s: failed to set first compression gain\n", __func__);
		return err;
	}
...
static int imx585_set_gc_gain_point_second(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;

	err = imx585_write_reg(s_data, ACMP2_EXP, (u8)val);
	if (err) {
		dev_err(dev, "%s: failed to set second compression gain\n", __func__);
		return err;
	}
...
static int imx585_set_exposure_threshold_low(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;

	err = imx585_write_buffered_reg(s_data, EXP_TH_L_LOW, 2, val);
	if (err) {
		dev_err(dev, "%s: failed to set low threshold\n", __func__);
		return err;
	}
...
static int imx585_set_exposure_threshold_high(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;

	err = imx585_write_buffered_reg(s_data, EXP_TH_H_LOW, 2, val);
	if (err) {
		dev_err(dev, "%s: failed to set high threshold\n", __func__);
		return err;
	}
...
static int imx585_set_gain_offset(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;

	err = imx585_write_reg(s_data, EXP_GAIN, (u8)val);
	if (err) {
		dev_err(dev, "%s: failed to set gain offset\n", __func__);
		return err;
	}
...
static int imx585_set_adc_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct imx585 *priv = (struct imx585 *)tegracam_get_privdata(tc_dev);
	struct camera_common_data *s_data = tc_dev->s_data;
	struct sensor_control_properties *ctrlprops = NULL;
	...
	if (val) {
		ctrlprops->min_gain_val = 0;
		ctrlprops->max_gain_val = IMX585_MAX_GAIN_CLEAR_HDR;
		priv->min_shr = IMX585_MIN_SHR0_CLEAR_HDR_LENGTH;
	} else {
		ctrlprops->min_gain_val = 0;
		ctrlprops->max_gain_val = IMX585_MAX_GAIN;
		priv->min_shr = IMX585_MIN_SHR0_LENGTH;
	}

	/* Update gain range */
	fr_update_ctrl_range(tc_dev, TEGRA_CAMERA_CID_GAIN,
				ctrlprops->min_gain_val, ctrlprops->max_gain_val);

	return 0;
}

static int imx585_configure_adc_mode(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx585 *priv = (struct imx585 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	...
	if (fr_get_v4l2_ctrl_value(tc_dev, TEGRA_CAMERA_CID_ADC_MODE, CURRENT)) {
		...
		case MEDIA_BUS_FMT_SRGGB10_1X10:
			err = imx585_write_table(priv,
					mode_table[IMX585_CLEAR_HDR_10BIT_MODE]);
			break;
		case MEDIA_BUS_FMT_SRGGB12_1X12:
			err = imx585_write_table(priv,
					mode_table[IMX585_CLEAR_HDR_12BIT_MODE]);
			break;
		case MEDIA_BUS_FMT_SRGGB16_1X16:
			err = imx585_write_table(priv,
					mode_table[IMX585_CLEAR_HDR_12BIT_MODE]);
			err = imx585_write_table(priv,
					mode_table[IMX585_CLEAR_HDR_16BIT_MODE]);
			break;
		default:
...
static int imx585_set_test_pattern(struct tegracam_device *tc_dev, u32 val)
{
	...
	if (val) {
		err = imx585_write_table(priv, mode_table[IMX585_EN_PATTERN_GEN]);
		if (err)
			goto fail;
		...
		err = imx585_write_table(priv, mode_table[IMX585_DIS_PATTERN_GEN]);
		if (err)
			goto fail;
	}
...
static int imx585_update_framerate_range(struct tegracam_device *tc_dev)
{
	...
	if (imx585_is_binning_mode(s_data))
		priv->min_frame_length = IMX585_DEFAULT_HEIGHT
						+ IMX585_MIN_FRAME_LENGTH_DELTA;
	else
		priv->min_frame_length = s_data->fmt_height
						+ IMX585_MIN_FRAME_LENGTH_DELTA;

	max_framerate = (IMX585_G_FACTOR * IMX585_M_FACTOR) /
				 (priv->min_frame_length * priv->line_time);

	if (fr_get_v4l2_ctrl_value(tc_dev, TEGRA_CAMERA_CID_ADC_MODE, CURRENT) &&
		(s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB10_1X10
		|| s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB12_1X12)) {
		max_framerate = max_framerate >> 1;
	}
...
static int imx585_set_operation_mode(struct tegracam_device *tc_dev, u32 val)
{
	return 0;
}

static bool imx585_find_broadcast_sensor(struct imx585 *broadcast_private)
{
	struct imx585 *current_private;

	list_for_each_entry(current_private, &imx585_sensor_list, entry) {
...
static void imx585_enable_second_slave_address(struct imx585 *ack_private)
{
	struct imx585 *current_private;
	int err;
	...
	list_for_each_entry(current_private, &imx585_sensor_list, entry) {
		...
		err = imx585_write_reg(current_private->s_data,
								SECOND_SLAVE_ADD, 1);
		if (err)
	...
	err = imx585_write_reg(ack_private->s_data,
								SECOND_SLAVE_ADD, 3);
	if (err)
...
static void imx585_disable_second_slave_address(void)
{
	struct imx585 *current_private;
	int err;
...
static void imx585_configure_second_slave_address(void)
{
	struct imx585 broadcast_private = {};

	if (imx585_find_broadcast_sensor(&broadcast_private))
		imx585_enable_second_slave_address(&broadcast_private);
	else
		imx585_disable_second_slave_address();
}

static int imx585_set_broadcast_ctrl(struct tegracam_device *tc_dev,
					struct v4l2_ctrl *ctrl)
{
	struct imx585 *priv = (struct imx585 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	...
	imx585_configure_second_slave_address();
...
static int imx585_set_custom_ctrls(struct v4l2_ctrl *ctrl)
{
	...
	switch (ctrl->id) {
	case TEGRA_CAMERA_CID_ADC_MODE:
		err = ops->set_adc_mode(tc_dev, *ctrl->p_new.p_u32);
		break;
	case TEGRA_CAMERA_CID_GAIN_OFFSET:
		err = ops->set_gain_offset(tc_dev, *ctrl->p_new.p_u32);
		break;
	case TEGRA_CAMERA_CID_EXPOSURE_THRESHOLD:
		err = ops->set_exposure_threshold(tc_dev, *ctrl->p_new.p_u32);
		break;
	case TEGRA_CAMERA_CID_EXPOSURE_THRESHOLD_HIGH:
		err = ops->set_exposure_threshold_high(tc_dev, *ctrl->p_new.p_u32);
		break;
	case TEGRA_CAMERA_CID_GC_START_POINT_FIRST:
		err = ops->set_gc_point_first(tc_dev, *ctrl->p_new.p_u32);
		break;
	case TEGRA_CAMERA_CID_GC_START_POINT_SECOND:
		err = ops->set_gc_point_second(tc_dev, *ctrl->p_new.p_u32);
		break;
	case TEGRA_CAMERA_CID_GC_GAIN_POINT_FIRST:
		err = ops->set_gc_gain_point_first(tc_dev, *ctrl->p_new.p_u32);
		break;
	case TEGRA_CAMERA_CID_GC_GAIN_POINT_SECOND:
		err = ops->set_gc_gain_point_second(tc_dev, *ctrl->p_new.p_u32);
		break;
	default:
...
static int imx585_power_off(struct camera_common_data *s_data)
{
	...
	err = imx585_write_reg(s_data, XVS_XHS_DRV, 0xF);
	if (err)
...
static int imx585_communication_verify(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx585 *priv = (struct imx585 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	...
	err = imx585_read_buffered_reg(s_data, VMAX_LOW, 2, &vmax);
	if (err) {
...
static int imx585_set_pixel_format(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
	struct imx585 *priv = (struct imx585 *)tegracam_get_privdata(tc_dev);
	struct camera_common_data *s_data = tc_dev->s_data;
	...
	switch (s_data->colorfmt->code) {
	case MEDIA_BUS_FMT_SRGGB10_1X10:
		err = imx585_write_table(priv, mode_table[IMX585_10BIT_MODE]);
		break;
	case MEDIA_BUS_FMT_SRGGB12_1X12:
		err = imx585_write_table(priv, mode_table[IMX585_12BIT_MODE]);
		break;
	case MEDIA_BUS_FMT_SRGGB16_1X16:
		err = imx585_write_table(priv, mode_table[IMX585_12BIT_MODE]);
		break;
...
static int imx585_set_csi_lane_mode(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;

	if (s_data->numlanes == IMX585_TWO_LANE_MODE) {
		err = imx585_write_reg(s_data, LANEMODE, 1);
		if (err) {
...
static int imx585_adjust_hmax_register(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;
	u64 hmax = 366;
	u8 csi_lane_coef = IMX585_4_CSI_LANES / s_data->numlanes;

	dev_dbg(dev, "%s:++\n", __func__);

	switch (fr_get_v4l2_ctrl_value(tc_dev, TEGRA_CAMERA_CID_DATA_RATE, CURRENT)) {
	case IMX585_2376_MBPS:
		hmax = 366;
		break;
	case IMX585_2079_MBPS:
		hmax = 366;
		break;
	case IMX585_1782_MBPS:
		hmax = 550 * csi_lane_coef;
		break;
	case IMX585_1440_MBPS:
		if (s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB10_1X10)
			hmax = 550 * csi_lane_coef;
		else
			hmax = 660 * csi_lane_coef;
		break;
	case IMX585_1188_MBPS:
		hmax = 660 * csi_lane_coef;
		break;
	case IMX585_891_MBPS:
		hmax = 1100;
		break;
	case IMX585_720_MBPS:
		if (s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB10_1X10)
			hmax = 1100;
		else
			hmax = 1320;
		break;
	case IMX585_594_MBPS:
		hmax = 1320;
		break;
	}

	if (fr_get_v4l2_ctrl_value(tc_dev, TEGRA_CAMERA_CID_ADC_MODE, CURRENT)) {
		hmax = (hmax < 550) ? 550 : hmax;
		if (s_data->numlanes == 2)
			hmax = (hmax < 660) ? 660 : hmax;
	}

	err = imx585_write_buffered_reg(s_data, HMAX_LOW, 2, hmax);
	if (err) {
...
static int imx585_force_predefined_data_rate(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	struct v4l2_ctrl *ctrl;
	int err = 0;
	int forced_data_rate = -1;

	ctrl = fr_find_v4l2_ctrl(tc_dev, TEGRA_CAMERA_CID_ADC_MODE);
	if (ctrl == NULL)
		return 0;
	...
	if (fr_get_v4l2_ctrl_value(tc_dev, TEGRA_CAMERA_CID_ADC_MODE, CURRENT)) {
		if (s_data->numlanes == IMX585_TWO_LANE_MODE) {
			forced_data_rate = IMX585_2376_MBPS;
			goto modify_ctrl;
		} else {
			forced_data_rate = IMX585_1782_MBPS;
			goto modify_ctrl;
		}
	}
...
modify_ctrl:
	if (fr_get_v4l2_ctrl_value(tc_dev, TEGRA_CAMERA_CID_DATA_RATE, CURRENT) != forced_data_rate) {
		dev_info(dev, "%s: Data rate changed due to static libargus limitation\n", __func__);
		*ctrl->p_new.p_s64 = forced_data_rate;
		*ctrl->p_cur.p_s64 = forced_data_rate;
	}

	return err;
}

static int imx585_verify_data_rate(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	struct v4l2_ctrl *ctrl;
	...
	if (s_data->numlanes == IMX585_TWO_LANE_MODE) {
		...
		case IMX585_594_MBPS:
			...
			if (s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB10_1X10) {
				data_rate = IMX585_1440_MBPS;
				goto modify_ctrl;
			} else {
				data_rate = IMX585_1782_MBPS;
				goto modify_ctrl;
			}
			break;
		case IMX585_1782_MBPS:
		...
		case IMX585_2079_MBPS:
			if (s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB12_1X12) {
				data_rate = IMX585_1782_MBPS;
				goto modify_ctrl;
			} else if (fr_get_v4l2_ctrl_value(tc_dev, TEGRA_CAMERA_CID_ADC_MODE, CURRENT)
						&& s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB10_1X10) {
				data_rate = IMX585_1440_MBPS;
				goto modify_ctrl;
			}
...
static int imx585_set_data_rate(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;
	...
	err = imx585_verify_data_rate(tc_dev);
	if (err)
		goto fail;

	err = imx585_force_predefined_data_rate(tc_dev);
	if (err)
		goto fail;

	if (s_data->power->state == SWITCH_ON) {
		err = imx585_write_reg(s_data, DATARATE_SEL,
...
static int imx585_set_sync_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err = 0;
	...
	if (s_data->power->state == SWITCH_ON) {
		err = imx585_write_reg(s_data, EXTMODE, extmode);
		if (err)
...
static int imx585_configure_triggering_pins(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err = 0;
	...
	err = imx585_write_reg(s_data, XVS_XHS_DRV, xvs_xhs_drv);
	if (err) {
 ...
 */
static int imx585_check_unsupported_mode(struct camera_common_data *s_data,
...
static int imx585_after_set_pixel_format(struct camera_common_data *s_data)
{
	struct device *dev = s_data->dev;
	...
	if (priv->current_pixel_format != s_data->colorfmt->code) {
		...
		switch (s_data->colorfmt->code) {
		case MEDIA_BUS_FMT_SRGGB10_1X10:
			err = imx585_update_ctrl(tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
					 (*ctrl->p_cur.p_s64 >> 2),
					IMX585_DEFAULT_BLACK_LEVEL_10BPP,
					0, IMX585_MAX_BLACK_LEVEL_10BPP);
		...
		case MEDIA_BUS_FMT_SRGGB16_1X16:
			err = imx585_update_ctrl(tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
					 (*ctrl->p_cur.p_s64 << 2),
					IMX585_DEFAULT_BLACK_LEVEL_12BPP,
					0, IMX585_MAX_BLACK_LEVEL_12BPP);
...
static int imx585_set_mode(struct tegracam_device *tc_dev)
{
	...
	err = imx585_write_table(priv, mode_table[IMX585_INIT_SETTINGS]);
	if (err) {
		dev_err(dev, "%s: unable to initialize sensor settings\n", __func__);
		return err;
	}

	err = imx585_set_csi_lane_mode(tc_dev);
	if (err) {
		dev_err(dev, "%s: error setting CSI lane mode\n", __func__);
		return err;
	}

	err = imx585_set_pixel_format(tc_dev);
	if (err) {
	...
	err = imx585_set_operation_mode(tc_dev, fr_get_v4l2_ctrl_value(tc_dev,
	...
	err = imx585_configure_adc_mode(tc_dev);
	if (err) {
		dev_err(dev, "%s: unable to configure adc mode\n", __func__);
		return err;
	}

	err = imx585_set_sync_mode(tc_dev, fr_get_v4l2_ctrl_value(tc_dev,
	...
	err = imx585_configure_triggering_pins(tc_dev);
	if (err) {
		dev_err(dev, "%s: unable configure XVS/XHS pins\n", __func__);
		return err;
	}

	err = imx585_set_data_rate(tc_dev, fr_get_v4l2_ctrl_value(tc_dev,
	...
	err = imx585_set_test_pattern(tc_dev, fr_get_v4l2_ctrl_value(tc_dev,
	...
	err = imx585_adjust_hmax_register(tc_dev);
	if (err) {
	...
	err = imx585_set_compression(tc_dev);
	if (err) {
		dev_err(dev, "%s: unable to set compresion\n", __func__);
		return err;
	}
	...
	err = imx585_update_framerate_range(tc_dev);
	if (err)
		return err;
...
static int imx585_start_streaming(struct tegracam_device *tc_dev)
{
	...
	err = imx585_write_table(priv, mode_table[IMX585_MODE_START_STREAM]);
	if (err)
		goto exit;
	...
	err = imx585_write_reg(s_data, XMSTA, xmsta);
	if (err)
		goto exit;
...
static int imx585_stop_streaming(struct tegracam_device *tc_dev)
{
	...
	err = imx585_write_table(priv, mode_table[IMX585_MODE_STOP_STREAM]);
	if (err)
		return err;

	usleep_range(priv->frame_length * priv->line_time / IMX585_K_FACTOR,
		priv->frame_length * priv->line_time / IMX585_K_FACTOR + 1000);
...
static struct camera_common_sensor_ops imx585_common_ops = {
	...
	.check_unsupported_mode = imx585_check_unsupported_mode,
	.after_set_pixel_format = imx585_after_set_pixel_format,
	.init_private_controls = imx585_ctrls_init,
};

static int imx585_gmsl_serdes_setup(struct imx585 *priv)
{
	int err = 0;
	int des_err = 0;
	struct device *dev;
...
static void imx585_gmsl_serdes_reset(struct imx585 *priv)
{
	mutex_lock(&serdes_lock__);
...
static int imx585_board_setup(struct imx585 *priv)
{
	...
	if (!(strcmp(s_data->pdata->gmsl, "gmsl"))) {
		...
		err = imx585_gmsl_serdes_setup(priv);
		if (err) {
	...
	err = imx585_communication_verify(priv->tc_dev);
	if (err) {
...
static struct tegracam_ctrl_ops imx585_ctrl_ops = {
	...
	.set_conversion_gain = imx585_set_conversion_gain,
	...
	.set_data_rate = imx585_set_data_rate,
	.set_adc_mode = imx585_set_adc_mode,
	.set_exposure_threshold = imx585_set_exposure_threshold_low,
	.set_exposure_threshold_high = imx585_set_exposure_threshold_high,
	.set_gain_offset = imx585_set_gain_offset,
	.set_gc_point_first = imx585_set_gc_point_first,
	.set_gc_point_second = imx585_set_gc_point_second,
	.set_gc_gain_point_first = imx585_set_gc_gain_point_first,
	.set_gc_gain_point_second = imx585_set_gc_gain_point_second,
	.set_operation_mode = imx585_set_operation_mode,
	.set_sync_mode = imx585_set_sync_mode,
	.set_broadcast_ctrl = imx585_set_broadcast_ctrl,
	.set_black_level = imx585_set_black_level,
};
...
#endif
{
	...
	priv = devm_kzalloc(dev,
			sizeof(struct imx585), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;
	...
	err = imx585_update_ctrl(tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
		 IMX585_DEFAULT_BLACK_LEVEL_12BPP, IMX585_DEFAULT_BLACK_LEVEL_12BPP,
		 0, IMX585_MAX_BLACK_LEVEL_12BPP);
	if (err)
		return err;

	err = imx585_update_ctrl(tc_dev, TEGRA_CAMERA_CID_TEST_PATTERN,
		0, 0, 0, (ARRAY_SIZE(imx585_test_pattern_menu)-1));
	if (err)
		return err;

	err = imx585_update_ctrl(tc_dev, TEGRA_CAMERA_CID_DATA_RATE,
		0, 0, 0, (ARRAY_SIZE(imx585_data_rate_menu)-1));
	if (err)
		return err;

	list_add_tail(&priv->entry, &imx585_sensor_list);
