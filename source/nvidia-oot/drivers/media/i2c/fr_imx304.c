 ...
 *
 * fr_imx304.c - Framos fr_imx304.c driver
 */
...
#include "fr_imx304_mode_tbls.h"
#include "media/fr_sensor_common.h"
#include "media/fr_lifmd_lvds2mipi_1.h"

#define IMX304_K_FACTOR 1000LL
#define IMX304_M_FACTOR 1000000LL
#define IMX304_G_FACTOR 1000000000LL
#define IMX304_T_FACTOR 1000000000000LL

#define IMX304_MAX_GAIN_DEC 480
#define IMX304_MAX_GAIN_DB 48

#define IMX304_MAX_BLACK_LEVEL 511
#define IMX304_DEFAULT_BLACK_LEVEL 240

#define IMX304_DEFAULT_LINE_TIME 14020

#define IMX304_MIN_SHS_LENGTH 8
#define IMX304_INTEGRATION_OFFSET 14
#define IMX304_MIN_INTEGRATION_LINES 1

#define IMX304_INCK 74250000LL

#define IMX304_LVDS_NUM_CHANNELS 8
#define IMX304_READOUT_LINE_SKIP 11

#define IMX304_TGPD_ALL_PIXEL 3048

#define IMX304_CAMERA_CID_BASE				(V4L2_CTRL_CLASS_CAMERA | 0x6000)
#define IMX304_CAMERA_CID_VERIFY_FW_COMPATIBILITY	(IMX304_CAMERA_CID_BASE + 0)
#define IMX304_CAMERA_CID_TG_SET_OPERATION_MODE		(IMX304_CAMERA_CID_BASE + 1)
#define IMX304_CAMERA_CID_TG_EXPAND_TRIGGER_EXPOSURE	(IMX304_CAMERA_CID_BASE + 2)
#define IMX304_CAMERA_CID_TG_DELAY_FRAME		(IMX304_CAMERA_CID_BASE + 3)
...
static int imx304_set_custom_ctrls(struct v4l2_ctrl *ctrl);
static const struct v4l2_ctrl_ops imx304_custom_ctrl_ops = {
	.s_ctrl = imx304_set_custom_ctrls,
};

static const char * const imx304_test_pattern_menu[] = {
	[0] = "No pattern",
...
static struct v4l2_ctrl_config imx304_custom_ctrl_list[] = {
	{
		.ops = &imx304_custom_ctrl_ops,
		.id = IMX304_CAMERA_CID_VERIFY_FW_COMPATIBILITY,
	...
	{
		.ops = &imx304_custom_ctrl_ops,
		.id = IMX304_CAMERA_CID_TG_SET_OPERATION_MODE,
	...
	{
		.ops = &imx304_custom_ctrl_ops,
		.id = IMX304_CAMERA_CID_TG_EXPAND_TRIGGER_EXPOSURE,
	...
	{
		.ops = &imx304_custom_ctrl_ops,
		.id = IMX304_CAMERA_CID_TG_DELAY_FRAME,
		.name = "Delay frame",
		.type = V4L2_CTRL_TYPE_INTEGER64,
		.flags = V4L2_CTRL_FLAG_SLIDER,
		.min = 0,
...
static int imx304_read_buffered_reg(struct camera_common_data *s_data,
			u16 addr_low, u8 number_of_registers, u64 *val)
{
	...
	*val = 0;
	...
	if (!s_data->group_hold_active) {
		err = imx304_write_reg(s_data, REGHOLD, 0x01);
		if (err) {
	...
	for (i = 0; i < number_of_registers; i++) {
		err = imx304_read_reg(s_data, addr_low + i, &reg);
		*val += reg << (i * 8);
		if (err) {
	...
	if (!s_data->group_hold_active) {
		err = imx304_write_reg(s_data, REGHOLD, 0x00);
		if (err) {
...
static int imx304_write_buffered_reg(struct camera_common_data *s_data,
				u16 addr_low, u8 number_of_registers, u64 val)
{
	int err, i;
	struct device *dev = s_data->dev;
	...
	for (i = 0; i < number_of_registers; i++) {
		err = imx304_write_reg(s_data, addr_low + i,
						(u8)(val >> (i * 8)));
		if (err) {
...
static int imx304_write_table(struct imx304 *priv, const imx304_reg table[])
{
...
static int imx304_set_group_hold(struct tegracam_device *tc_dev, bool val)
{
	...
	err = imx304_write_reg(s_data, REGHOLD, val);
	if (err) {
		dev_err(dev, "%s: GRP_PARAM_HOLD error\n", __func__);
		return err;
	}
...
static int imx304_update_ctrl(struct tegracam_device *tc_dev, int ctrl_id,
								...
								u64 max_val)
{
	...
	if (ctrl) {
		...
		case TEGRA_CAMERA_CID_TEST_PATTERN:
			ctrl->qmenu = imx304_test_pattern_menu;
			ctrl->maximum = max_val;
			break;
...
static int imx304_set_black_level(struct tegracam_device *tc_dev, s64 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;

	err = imx304_write_buffered_reg(s_data, BLKLEVEL_LOW, 2, val);
	if (err) {
...
static int imx304_set_gain(struct tegracam_device *tc_dev, s64 val)
{
	...
	gain = val * IMX304_MAX_GAIN_DEC /
				(IMX304_MAX_GAIN_DB *
					mode->control_properties.gain_factor);

	err = imx304_write_buffered_reg(s_data, GAIN_LOW, 2, gain);
	if (err) {
...
static int imx304_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
	...
	u32 integration_offset = IMX304_INTEGRATION_OFFSET;
	u32 reg_shs, min_reg_shs = IMX304_MIN_SHS_LENGTH;
	...
	integration_time_line = ((val - integration_offset)
					* IMX304_K_FACTOR) / priv->line_time;

	reg_shs = priv->frame_length - integration_time_line;

	if (reg_shs < min_reg_shs)
		reg_shs = min_reg_shs;
	else if (reg_shs > (priv->frame_length - IMX304_MIN_INTEGRATION_LINES))
		reg_shs = priv->frame_length - IMX304_MIN_INTEGRATION_LINES;

	err = lifmd_lvds2mipi_1_tg_set_trigger_exposure(priv->lifmd_lvds2mipi_1,
							(u32)val, s_data);
	err = imx304_write_buffered_reg(s_data, SHS1_LOW, 3, reg_shs);
	if (err) {
...
static int imx304_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
	...
	frame_length = (((u64)mode->control_properties.framerate_factor *
				IMX304_G_FACTOR) / (val * priv->line_time));
	...
	exposure_min_range = IMX304_MIN_INTEGRATION_LINES
					* priv->line_time / IMX304_K_FACTOR;
	exposure_min_range += IMX304_INTEGRATION_OFFSET;
	exposure_max_range = (priv->frame_length - IMX304_MIN_SHS_LENGTH)
					* priv->line_time / IMX304_K_FACTOR;
	...
	err = lifmd_lvds2mipi_1_tg_set_frame_width(priv->lifmd_lvds2mipi_1,
						IMX304_G_FACTOR / (val / 1000));
	err = imx304_write_buffered_reg(s_data, VMAX_LOW, 3, priv->frame_length);
	if (err) {
		dev_err(dev, "%s: failed to set frame length\n", __func__);
		return err;
	}
...
static int imx304_set_test_pattern(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;
	...
	if (val) {
		err = imx304_write_reg(s_data, 0x3238, (u8)(((val<<3) | 0x05)));
		if (err)
			goto fail;

		err = imx304_write_reg(s_data, 0x3354, 0x00);
		if (err)
			goto fail;

		err = imx304_write_reg(s_data, 0x3355, 0x00);
		if (err)
			goto fail;
	} else {
		err = imx304_write_reg(s_data, 0x3238, 0x06);
		if (err)
			goto fail;
	}
...
static int imx304_update_framerate_range(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx304 *priv = (struct imx304 *)tegracam_get_privdata(tc_dev);
	...
	switch (s_data->mode) {
	case IMX304_MODE_ROI_4096x2160:
	case IMX304_MODE_ROI_3840x2160:
		priv->min_frame_length = 2350;
		break;
	default:
		priv->min_frame_length = s_data->fmt_height
					+ IMX304_MIN_FRAME_LENGTH_DELTA;
	}

	max_framerate = (IMX304_G_FACTOR * IMX304_M_FACTOR) /
...
static int imx304_set_operation_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct imx304 *priv = (struct imx304 *)tc_dev->priv;
...
static int imx304_set_shutter_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct imx304 *priv = (struct imx304 *)tc_dev->priv;
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	...
	case SEQ_TRIGGER:
		if ((fr_get_v4l2_ctrl_value(tc_dev, TEGRA_CAMERA_CID_OPERATION_MODE,
			...
			dev_warn(dev,
				...
				__func__);
			val = NORMAL_EXPO;
			imx304_update_ctrl(tc_dev, TEGRA_CAMERA_CID_SHUTTER_MODE,
						val, 0, 0, SEQ_TRIGGER);
			break;
		}
		trigen = 1;
	...
	if (s_data->power->state == SWITCH_ON) {
		err = imx304_write_reg(s_data, TRIGEN, trigen);
		err |= imx304_write_reg(s_data, VINT_EN, vint_en);
		if (err) {
			dev_err(dev, "%s: error setting shutter - exposure mode\n",
								__func__);
		return err;
		}
	}
...
static int imx304_set_custom_ctrls(struct v4l2_ctrl *ctrl)
{
	...
	switch (ctrl->id) {
	case IMX304_CAMERA_CID_VERIFY_FW_COMPATIBILITY:
		err = lifmd_lvds2mipi_1_verify_fw_compatibility(priv->lifmd_lvds2mipi_1,
								ctrl, priv->s_data);
		break;
	case IMX304_CAMERA_CID_TG_SET_OPERATION_MODE:
		err = lifmd_lvds2mipi_1_tg_set_operation_mode(priv->lifmd_lvds2mipi_1, ctrl);
		break;
	case IMX304_CAMERA_CID_TG_EXPAND_TRIGGER_EXPOSURE:
		err = lifmd_lvds2mipi_1_tg_expand_trigger_exposure(priv->lifmd_lvds2mipi_1,
								*ctrl->p_new.p_u32,
								priv->s_data);
		break;
	case IMX304_CAMERA_CID_TG_DELAY_FRAME:
		err = lifmd_lvds2mipi_1_tg_delay_frame(priv->lifmd_lvds2mipi_1,
							*ctrl->p_new.p_u32,
							priv->s_data);
		break;
	default:
...
static int imx304_calculate_line_time(struct tegracam_device *tc_dev)
{
	struct imx304 *priv = (struct imx304 *)tc_dev->priv;
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	...
	err = imx304_read_buffered_reg(s_data, HMAX_LOW, 2, &hmax);
	if (err) {
		dev_err(dev, "%s: unable to read hmax\n", __func__);
		return err;
	}

	priv->line_time = (hmax*IMX304_G_FACTOR) / (IMX304_INCK);
...
static int imx304_set_trigger_tgpd(struct tegracam_device *tc_dev)
{
	struct imx304 *priv = (struct imx304 *)tc_dev->priv;
	struct camera_common_data *s_data = tc_dev->s_data;

	switch (s_data->mode) {
	case IMX304_MODE_4112x3008:
		priv->lifmd_lvds2mipi_1->tg_params.t_tgpd = IMX304_TGPD_ALL_PIXEL;
		break;
	case IMX304_MODE_ROI_4096x2160:
	case IMX304_MODE_ROI_3840x2160:
		priv->lifmd_lvds2mipi_1->tg_params.t_tgpd = s_data->fmt_height
					+ IMX304_MIN_FRAME_LENGTH_DELTA;
	}
...
static int imx304_set_mode(struct tegracam_device *tc_dev)
{
	...
	err = imx304_write_table(priv, mode_table[IMX304_INIT_SETTINGS]);
	if (err) {
	...
	err = imx304_write_table(priv, mode_table[s_data->mode]);
	if (err) {
	...
	err = imx304_set_operation_mode(tc_dev, fr_get_v4l2_ctrl_value(tc_dev,
	...
	err = imx304_set_shutter_mode(tc_dev, fr_get_v4l2_ctrl_value(tc_dev,
				TEGRA_CAMERA_CID_SHUTTER_MODE, CURRENT));
	if (err) {
		dev_err(dev, "%s: unable to set shutter mode\n", __func__);
		return err;
	}

	ctrl = fr_find_v4l2_ctrl(tc_dev, IMX304_CAMERA_CID_VERIFY_FW_COMPATIBILITY);
	err = lifmd_lvds2mipi_1_verify_fw_compatibility(priv->lifmd_lvds2mipi_1,
							ctrl, priv->s_data);
	if (err) {
	...
	if (fr_get_v4l2_ctrl_value(tc_dev, IMX304_CAMERA_CID_TG_SET_OPERATION_MODE,
						CURRENT) != TG_DISABLED) {
		err = lifmd_lvds2mipi_1_tg_set_operation_mode(priv->lifmd_lvds2mipi_1,
					fr_find_v4l2_ctrl(tc_dev,
					IMX304_CAMERA_CID_TG_SET_OPERATION_MODE));
		if (err) {
		...
		err = lifmd_lvds2mipi_1_tg_expand_trigger_exposure(priv->lifmd_lvds2mipi_1,
					...
					IMX304_CAMERA_CID_TG_EXPAND_TRIGGER_EXPOSURE,
					CURRENT), s_data);
		if (err) {
			dev_err(dev, "%s: unable to set TG expand triger exposure\n",
								__func__);
		return err;
		}
		...
		err = lifmd_lvds2mipi_1_tg_delay_frame(priv->lifmd_lvds2mipi_1,
					fr_get_v4l2_ctrl_value(tc_dev,
					IMX304_CAMERA_CID_TG_DELAY_FRAME,
					CURRENT), s_data);
		if (err) {
	...
	err = imx304_set_trigger_tgpd(tc_dev);
	if (err)
		goto fail;

	err = imx304_set_test_pattern(tc_dev, fr_get_v4l2_ctrl_value(tc_dev,
	...
	err = imx304_calculate_line_time(tc_dev);
	if (err)
		goto fail;

	err = lifmd_lvds2mipi_1_tg_set_line_width(priv->lifmd_lvds2mipi_1, s_data);
	if (err)
		goto fail;

	err = imx304_update_framerate_range(tc_dev);
	if (err)
		goto fail;
...
static int imx304_start_streaming(struct tegracam_device *tc_dev)
{
	...
	err = imx304_write_reg(s_data, STANDBY, 0x00);

	usleep_range(1000, 1100);

	err |= lifmd_lvds2mipi_1_start(priv->lifmd_lvds2mipi_1);

	if ((fr_get_v4l2_ctrl_value(priv->tc_dev, TEGRA_CAMERA_CID_OPERATION_MODE,
						CURRENT)) == MASTER_MODE) {
		err |= imx304_write_reg(s_data, XMSTA, 0x00);
	} else {
...
static int imx304_stop_streaming(struct tegracam_device *tc_dev)
{
	...
	usleep_range(priv->frame_length * priv->line_time / IMX304_K_FACTOR,
		priv->frame_length * priv->line_time / IMX304_K_FACTOR + 1000);

	return 0;
}

static int imx304_ctrls_init(struct tegracam_device *tc_dev)
{
	struct imx304 *priv = (struct imx304 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	struct camera_common_data *s_data = priv->s_data;
	...
	numctrls = ARRAY_SIZE(imx304_custom_ctrl_list);

	for (i = 0; i < numctrls; i++) {
		ctrl_cfg = &imx304_custom_ctrl_list[i];
...
static struct camera_common_sensor_ops imx304_common_ops = {
	...
	.init_private_controls = imx304_ctrls_init,
};

static int imx304_gmsl_serdes_setup(struct imx304 *priv)
{
	int err = 0;
	int des_err = 0;
	struct device *dev;
...
static void imx304_gmsl_serdes_reset(struct imx304 *priv)
{
	mutex_lock(&serdes_lock__);
...
static int imx304_board_setup(struct imx304 *priv)
{
	...
	if (!strcmp(s_data->pdata->gmsl, "gmsl")) {
		...
		err = imx304_gmsl_serdes_setup(priv);
		if (err) {
	...
	lifmd_lvds2mipi_1_node = of_parse_phandle(node, "lifmd_lvds2mipi_1-device", 0);
	...
	lifmd_lvds2mipi_1_i2c = of_find_i2c_device_by_node(lifmd_lvds2mipi_1_node);
	...
	priv->lifmd_lvds2mipi_1->regmap = devm_regmap_init_i2c(priv->lifmd_lvds2mipi_1->i2c_client,
...
static struct tegracam_ctrl_ops imx304_ctrl_ops = {
	...
	.set_test_pattern = imx304_set_test_pattern,
	.set_operation_mode = imx304_set_operation_mode,
	.set_shutter_mode = imx304_set_shutter_mode,
	.set_black_level = imx304_set_black_level,
};

#if defined(NV_I2C_DRIVER_STRUCT_PROBE_WITHOUT_I2C_DEVICE_ID_ARG) /* Linux 6.3 */
static int imx304_probe(struct i2c_client *client)
#else
static int imx304_probe(struct i2c_client *client, const struct i2c_device_id *id)
#endif
{
	...
	priv->frame_length = IMX304_DEFAULT_HEIGHT
					+ IMX304_MIN_FRAME_LENGTH_DELTA;
	priv->line_time = IMX304_DEFAULT_LINE_TIME;
	priv->min_frame_length = IMX304_DEFAULT_HEIGHT
					+ IMX304_MIN_FRAME_LENGTH_DELTA;

	priv->lifmd_lvds2mipi_1 = devm_kzalloc(dev,
		sizeof(struct lifmd_lvds2mipi_1), GFP_KERNEL);
	if (!priv->lifmd_lvds2mipi_1)
		return -ENOMEM;

	priv->lifmd_lvds2mipi_1->tg_params.tg_mode = TG_DISABLED;
	priv->lifmd_lvds2mipi_1->tg_params.sync_logic = 1;
	priv->lifmd_lvds2mipi_1->tg_params.out_logic = 0;
	priv->lifmd_lvds2mipi_1->tg_params.xhs_min_active_width = 8;
	priv->lifmd_lvds2mipi_1->tg_params.xhs_clk_offset = 2;
	priv->lifmd_lvds2mipi_1->tg_params.frame_active_width = 2;
	priv->lifmd_lvds2mipi_1->tg_params.line_time = &priv->line_time;

	priv->lifmd_lvds2mipi_1->sensor_numch = IMX304_LVDS_NUM_CHANNELS;
	priv->lifmd_lvds2mipi_1->lifmd_lvds2mipi_1_readout_mode = lifmd_lvds2mipi_1_def_readout_mode;
	priv->lifmd_lvds2mipi_1->lifmd_lvds2mipi_1_readout_mode.line_skip = IMX304_READOUT_LINE_SKIP;
	...
	err = imx304_update_ctrl(tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
					IMX304_DEFAULT_BLACK_LEVEL,
					IMX304_DEFAULT_BLACK_LEVEL, 0,
					IMX304_MAX_BLACK_LEVEL);
	if (err)
		return err;

	err = imx304_update_ctrl(tc_dev, TEGRA_CAMERA_CID_TEST_PATTERN, 0, 0, 0,
				(ARRAY_SIZE(imx304_test_pattern_menu)-1));
	if (err)
		return err;
...
static int imx304_remove(struct i2c_client *client)
#else
static void imx304_remove(struct i2c_client *client)
#endif
{
	...
	priv = (struct imx304 *)s_data->priv;

	if (!(strcmp(s_data->pdata->gmsl, "gmsl")))
		imx304_gmsl_serdes_reset(priv);
