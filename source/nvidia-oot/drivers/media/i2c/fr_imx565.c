 ...
 *
 * fr_imx565.c - Framos fr_imx565.c driver
 */
...
static inline int imx565_read_reg(struct camera_common_data *s_data, u16 addr,
					u8 *val)
{
	int err = 0;
	u32 reg_val = 0;
...
static int imx565_write_reg(struct camera_common_data *s_data, u16 addr, u8 val)
{
	int err;
					 ...
					 u64 val)
{
	...
	for (i = 0; i < number_of_registers; i++) {
		err = imx565_write_reg(s_data, addr_low + i,
					 (u8)(val >> (i * 8)));
		if (err) {
...
static int imx565_write_table(struct imx565 *priv, const imx565_reg table[])
{
...
static int imx565_update_ctrl(struct tegracam_device *tc_dev, int ctrl_id,
				...
				u64 max_val)
{
	...
	if (ctrl) {
		...
		case TEGRA_CAMERA_CID_TEST_PATTERN:
			ctrl->qmenu = imx565_test_pattern_menu;
			ctrl->maximum = max_val;
			break;
		case TEGRA_CAMERA_CID_DATA_RATE:
			ctrl->qmenu = imx565_data_rate_menu;
			ctrl->maximum = max_val;
			break;
		}
	}
...
static int imx565_set_gain(struct tegracam_device *tc_dev, s64 val)
{
	...
	gain = val * IMX565_MAX_GAIN_DEC /
		 (IMX565_MAX_GAIN_DB * mode->control_properties.gain_factor);
...
static int imx565_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
	...
	integration_time_line =
		((val - integration_offset) * IMX565_K_FACTOR) / priv->line_time;
...
static int imx565_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
	...
	frame_length = (((u64)mode->control_properties.framerate_factor * IMX565_G_FACTOR) /
			(val * priv->line_time));
	...
	exposure_min_range =
		IMX565_MIN_INTEGRATION_LINES * priv->line_time / IMX565_K_FACTOR;
...
static int imx565_update_framerate_range(struct tegracam_device *tc_dev)
{
	...
	case IMX565_MODE_4128x3008:
		priv->min_frame_length =
			IMX565_DEFAULT_HEIGHT + gmrwt + gmtwt + gsdly + 86;
		break;
	case IMX565_MODE_ROI_3840x2160:
		priv->min_frame_length =
			IMX565_ROI_MODE_HEIGHT + gmrwt + gmtwt + gsdly + 86;
		break;
	case IMX565_MODE_BINNING_CROP_1920x1080:
		priv->min_frame_length = IMX565_BINNING_CROP_MODE_HEIGHT +
					 gmrwt + gmtwt + gsdly + 56;
		break;
	}
...
static int imx565_set_shutter_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct device *dev = tc_dev->dev;
	struct v4l2_ctrl *ctrl =
	...
	if (cid_operation_mode == MASTER_MODE && val == SEQ_TRIGGER) {
		dev_warn(dev,
	...
	if (cid_operation_mode == SLAVE_MODE && val == FAST_TRIGGER) {
		dev_warn(dev,
...
imx565_parse_dt(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
...
static int imx565_set_dep_registers(struct tegracam_device *tc_dev)
{
	...
	s64 cid_data_rate = fr_get_v4l2_ctrl_value(tc_dev,
						TEGRA_CAMERA_CID_DATA_RATE,
						CURRENT);

	gaindly = (s_data->mode == IMX565_MODE_BINNING_CROP_1920x1080) ? 0x04
									: 0x02;

	switch (cid_data_rate) {
	case IMX565_1188_MBPS:
		switch (s_data->colorfmt->code) {
		case MEDIA_BUS_FMT_SRGGB8_1X8:
			gmrwt = (s_data->mode == IMX565_MODE_BINNING_CROP_1920x1080)
				? (s_data->numlanes == IMX565_MAX_CSI_LANES
								? 0x08 : 0x04)
				: (s_data->numlanes == IMX565_MAX_CSI_LANES
								? 0x04 : 0x02);
			gmtwt = (s_data->mode == IMX565_MODE_BINNING_CROP_1920x1080)
				? (s_data->numlanes == IMX565_MAX_CSI_LANES
								? 0x30 : 0x1C)
				: (s_data->numlanes == IMX565_MAX_CSI_LANES
								? 0x1A : 0x0E);
			gsdly = (s_data->mode == IMX565_MODE_BINNING_CROP_1920x1080)
				? (s_data->numlanes == IMX565_MAX_CSI_LANES
								? 0x18 : 0x0C)
				: (s_data->numlanes == IMX565_MAX_CSI_LANES
								? 0x0C : 0x06);
...
static int imx565_adjust_hmax_register(struct tegracam_device *tc_dev)
{
	...
	case IMX565_1188_MBPS:
		switch (s_data->colorfmt->code) {
		case MEDIA_BUS_FMT_SRGGB8_1X8:
			hmax = (s_data->mode == IMX565_MODE_BINNING_CROP_1920x1080)
				? (s_data->numlanes == IMX565_MAX_CSI_LANES
					 ? 0x129
					 : 0x22A)
				: (s_data->numlanes == IMX565_MAX_CSI_LANES
...
static int imx565_set_data_rate(struct tegracam_device *tc_dev, u32 val)
{
	struct imx565 *priv = (struct imx565 *)tegracam_get_privdata(tc_dev);
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
...
static int imx565_configure_shutter(struct tegracam_device *tc_dev)
{
	...
	u8 trigen = 0;
	u8 vint_en = 0;
	s64 cid_shutter_mode = fr_get_v4l2_ctrl_value(tc_dev,
						TEGRA_CAMERA_CID_SHUTTER_MODE,
						CURRENT);

	switch (cid_shutter_mode) {
	case NORMAL_EXPO:
		trigen = 0;
		vint_en = 2;
	...
	case FAST_TRIGGER:
		if (fr_get_v4l2_ctrl_value(tc_dev, TEGRA_CAMERA_CID_OPERATION_MODE, CURRENT) == SLAVE_MODE) {
			dev_warn(dev,
...
static int imx565_after_set_pixel_format(struct camera_common_data *s_data)
{
	struct device *dev = s_data->dev;
	...
	if (priv->current_pixel_format != s_data->colorfmt->code) {
		...
		case MEDIA_BUS_FMT_SRGGB8_1X8:
			if (priv->current_pixel_format == MEDIA_BUS_FMT_SRGGB12_1X12)
				err = imx565_update_ctrl(
					tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
					(*ctrl->p_cur.p_s64 >> 4),
					IMX565_DEFAULT_BLACK_LEVEL_8BPP, 0,
					IMX565_MAX_BLACK_LEVEL_8BPP);
			else
				err = imx565_update_ctrl(
					tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
					(*ctrl->p_cur.p_s64 >> 2),
		...
		case MEDIA_BUS_FMT_SRGGB10_1X10:
			if (priv->current_pixel_format == MEDIA_BUS_FMT_SRGGB12_1X12)
				err = imx565_update_ctrl(
					tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
					(*ctrl->p_cur.p_s64 >> 2),
					IMX565_DEFAULT_BLACK_LEVEL_10BPP, 0,
					IMX565_MAX_BLACK_LEVEL_10BPP);
			else
				err = imx565_update_ctrl(
					tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
					(*ctrl->p_cur.p_s64 << 2),
		...
		case MEDIA_BUS_FMT_SRGGB12_1X12:
			if (priv->current_pixel_format == MEDIA_BUS_FMT_SRGGB10_1X10)
				err = imx565_update_ctrl(
					tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
					(*ctrl->p_cur.p_s64 << 2),
					IMX565_DEFAULT_BLACK_LEVEL_12BPP, 0,
					IMX565_MAX_BLACK_LEVEL_12BPP);
			else
				err = imx565_update_ctrl(
					tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
					(*ctrl->p_cur.p_s64 << 4),
...
static int imx565_start_streaming(struct tegracam_device *tc_dev)
{
	...
	err = imx565_write_reg(s_data, XMSTA, xmsta);
	if (err)
		goto exit;
...
static int imx565_gmsl_serdes_setup(struct imx565 *priv)
{
	int err = 0;
	int des_err = 0;
	struct device *dev;
...
static void imx565_gmsl_serdes_reset(struct imx565 *priv)
{
	mutex_lock(&serdes_lock__);
...
static int imx565_board_setup(struct imx565 *priv)
{
	...
	if (!(strcmp(s_data->pdata->gmsl, "gmsl"))) {
		...
		err = imx565_gmsl_serdes_setup(priv);
		if (err) {
	...
	err = imx565_calculate_line_time(priv->tc_dev);
	if (err) {
...
static int imx565_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	dev_dbg(&client->dev, "imx565 : open\n");
...
static struct tegracam_ctrl_ops imx565_ctrl_ops = {
	...
	.set_data_rate = imx565_set_data_rate,
...
#endif
{
	...
	priv = devm_kzalloc(dev, sizeof(struct imx565), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;
	...
	priv->frame_length = IMX565_DEFAULT_HEIGHT + IMX565_MIN_FRAME_DELTA;
	priv->current_pixel_format = MEDIA_BUS_FMT_SRGGB12_1X12;
	priv->line_time = IMX565_DEFAULT_LINE_TIME;
	priv->min_frame_length = IMX565_DEFAULT_HEIGHT + IMX565_MIN_FRAME_DELTA;
	...
	err = imx565_update_ctrl(tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
				 IMX565_DEFAULT_BLACK_LEVEL_12BPP,
				 IMX565_DEFAULT_BLACK_LEVEL_12BPP, 0,
				 IMX565_MAX_BLACK_LEVEL_12BPP);
	if (err)
		return err;

	err = imx565_update_ctrl(tc_dev, TEGRA_CAMERA_CID_TEST_PATTERN, 0, 0, 0,
				 (ARRAY_SIZE(imx565_test_pattern_menu) - 1));
	if (err)
		return err;

	err = imx565_update_ctrl(tc_dev, TEGRA_CAMERA_CID_DATA_RATE, 0, 0, 0,
				 (ARRAY_SIZE(imx565_data_rate_menu) - 1));
	if (err)
		return err;
...
static int imx565_remove(struct i2c_client *client)
#else
...
#endif
{
	...
	priv = (struct imx565 *)s_data->priv;

	if (!(strcmp(s_data->pdata->gmsl, "gmsl")))
		imx565_gmsl_serdes_reset(priv);
...
static const struct i2c_device_id imx565_id[] = {{"imx565", 0}, {}};
