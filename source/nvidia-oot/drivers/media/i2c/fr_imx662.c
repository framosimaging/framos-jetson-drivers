...
#define IMX662_HIGH_GAIN_REG_MIN 34
...
#define IMX662_4_CSI_LANES 4
#define IMX662_TWO_LANE_MODE 2

#define IMX662_INCK 74250000LL

#define IMX662_DOL2_MIN_SHR0_LENGTH (IMX662_DEFAULT_RHS1 + 5)
#define IMX662_DOL2_MIN_INTEGRATION_LINES 4


LIST_HEAD(imx662_sensor_list);
...
const char * const imx662_data_rate_menu[] = {
	[IMX662_1440_MBPS] = "1440 Mbps/lane",
	[IMX662_1188_MBPS] = "1188 Mbps/lane",
	[IMX662_891_MBPS] = "891 Mbps/lane",
	[IMX662_720_MBPS] = "720 Mbps/lane",
	[IMX662_594_MBPS] = "594 Mbps/lane",
};
...
static bool imx662_is_binning_mode(struct camera_common_data *s_data)
{
	switch (s_data->mode) {
	case IMX662_MODE_H2V2_BINNING:
	case IMX662_MODE_DOL_BINNING:
		return true;
	default:
		return false;
	}
...
static inline int imx662_read_reg(struct camera_common_data *s_data,
							u16 addr, u8 *val)
{
	int err = 0;
	u32 reg_val = 0;
...
static int imx662_write_reg(struct camera_common_data *s_data,
							u16 addr, u8 val)
{
	int err;
...
static int imx662_write_reg_broadcast(struct camera_common_data *s_data,
							u16 addr, u8 val)
{
	int err;
	struct device *dev = s_data->dev;
...
static int imx662_read_buffered_reg(struct camera_common_data *s_data,
			u16 addr_low, u8 number_of_registers, u64 *val)
{
	...
	*val = 0;
	...
	if (!s_data->group_hold_active) {
		err = imx662_write_reg(s_data, REGHOLD, 0x01);
		if (err) {
	...
	for (i = 0; i < number_of_registers; i++) {
		err = imx662_read_reg(s_data, addr_low + i, &reg);
		*val += reg << (i * 8);
		if (err) {
	...
	if (!s_data->group_hold_active) {
		err = imx662_write_reg(s_data, REGHOLD, 0x00);
		if (err) {
...
static int imx662_write_buffered_reg(struct camera_common_data *s_data,
			u16 addr_low, u8 number_of_registers, u64 val)
{
	int err, i;
	struct device *dev = s_data->dev;
	...
	for (i = 0; i < number_of_registers; i++) {
		err = imx662_write_reg(s_data, addr_low + i,
						(u8)(val >> (i * 8)));
		if (err) {
...
static int imx662_broadcast_buffered_reg(struct camera_common_data *s_data,
			u16 addr_low, u8 number_of_registers, u32 val)
{
	...
	if (!s_data->group_hold_active) {
		err = imx662_write_reg_broadcast(s_data, REGHOLD, 0x01);
		if (err) {
			dev_err(dev, "%s: GRP_PARAM_HOLD error\n", __func__);
			return err;
		}
	...
	for (i = 0; i < number_of_registers; i++) {
		err = imx662_write_reg_broadcast(s_data, addr_low + i,
					(u8)(val >> (i * 8)));
		if (err) {
			dev_err(dev, "%s: BUFFERED register write error\n",
								__func__);
	...
	if (!s_data->group_hold_active) {
		err = imx662_write_reg_broadcast(s_data, REGHOLD, 0x00);
		if (err) {
			dev_err(dev, "%s: GRP_PARAM_HOLD error\n", __func__);
			return err;
		}
...
static int imx662_write_table(struct imx662 *priv, const imx662_reg table[])
{
	...
	return regmap_util_write_table_8(s_data->regmap,
					table,
					NULL, 0,
					IMX662_TABLE_WAIT_MS,
					IMX662_TABLE_END);
}

static int imx662_set_group_hold(struct tegracam_device *tc_dev, bool val)
{
	...
	err = imx662_write_reg(s_data, REGHOLD, val);
	if (err) {
		dev_err(dev, "%s: GRP_PARAM_HOLD error\n", __func__);
		return err;
	}
...
static int imx662_update_ctrl(struct tegracam_device *tc_dev, int ctrl_id,
				u64 current_val, u64 default_val, u64 min_val,
								u64 max_val)
{
	struct imx662 *priv = (struct imx662 *)tegracam_get_privdata(tc_dev);
	...
	if (ctrl) {
		...
		case TEGRA_CAMERA_CID_TEST_PATTERN:
			ctrl->qmenu = imx662_test_pattern_menu;
			ctrl->maximum = max_val;
			break;
		case TEGRA_CAMERA_CID_DATA_RATE:
			ctrl->qmenu = imx662_data_rate_menu;
			ctrl->maximum = max_val;
			break;
		}
	}
...
static int imx662_in_dol_mode(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
...
static int imx662_set_black_level(struct tegracam_device *tc_dev, s64 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;
	...
	err = imx662_write_buffered_reg(s_data, BLKLEVEL_LOW, 2,
							black_level_reg);
	if (err) {
...
static int imx662_set_gain(struct tegracam_device *tc_dev, s64 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx662 *priv = (struct imx662 *)tc_dev->priv;
	struct device *dev = tc_dev->dev;
	const struct sensor_mode_properties *mode =
		&s_data->sensor_props.sensor_modes[s_data->mode];
	int err = 0;
	const int hdr_en = imx662_in_dol_mode(tc_dev);
	u32 gain;
	...
	gain = val * IMX662_MAX_GAIN_DEC /
				 (IMX662_MAX_GAIN_DB *
					 mode->control_properties.gain_factor);

	if (priv->broadcast_ctrl == BROADCAST) {
		err = imx662_broadcast_buffered_reg(s_data,
							GAIN_LOW, 2, gain);
	} else {
		err = imx662_write_buffered_reg(s_data,
							GAIN_LOW, 2, gain);
		if (hdr_en > 0)
			err |= imx662_write_buffered_reg(s_data,
							GAIN_1_LOW, 2, gain);
...
static int imx662_set_conversion_gain(struct tegracam_device *tc_dev, bool val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	const struct sensor_mode_properties *mode =
	...
	err = imx662_write_reg(s_data, FDG_SEL0, val);
	err |= imx662_write_reg(s_data, FDG_SEL1, val); // change conversion gain for dol2 also
	if (err) {
	...
	if (val) {
		err = imx662_read_buffered_reg(s_data, GAIN_LOW, 2, &curr_gain);
		if (err) {
			dev_err(dev, "%s: reading gain value error\n", __func__);
			return err;
		}
		min_high_gain = IMX662_HIGH_GAIN_REG_MIN * IMX662_MAX_GAIN_DB
			* mode->control_properties.gain_factor / IMX662_MAX_GAIN_DEC;

		if (curr_gain < IMX662_HIGH_GAIN_REG_MIN) {
			dev_warn(dev, "%s:Gain value too large for high conversion gain\n", __func__);
			curr_gain = IMX662_HIGH_GAIN_REG_MIN * IMX662_MAX_GAIN_DB
					* mode->control_properties.gain_factor
					/ IMX662_MAX_GAIN_DEC;
			err = imx662_set_gain(tc_dev, curr_gain);
			if (err) {
...
static int imx662_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx662 *priv = (struct imx662 *)tc_dev->priv;
	struct device *dev = tc_dev->dev;
	...
	min_shr0 = hdr_en ? IMX662_DOL2_MIN_SHR0_LENGTH :
						IMX662_MIN_SHR0_LENGTH;
	max_shr0 = hdr_en ? priv->frame_length -
					IMX662_DOL2_MIN_INTEGRATION_LINES :
			priv->frame_length - IMX662_MIN_INTEGRATION_LINES;
	...
	integration_time_line = (val * IMX662_K_FACTOR) / priv->line_time;
	...
	if (priv->broadcast_ctrl == BROADCAST)
		err = imx662_broadcast_buffered_reg(s_data, SHR0_LOW, 3, shr0);
	else
		err = imx662_write_buffered_reg(s_data, SHR0_LOW, 3, shr0);
	...
	dev_dbg(dev, "%s: set integration time: %lld [us], integration time: %d [line], shr0: %llu [line], frame length: %llu [line]\n",
		__func__, val, integration_time_line, shr0, priv->frame_length);

	dev_dbg(dev, "%s: min long exp: %lld [us], max long exp:%lld [us]\n",
		__func__, s_data->exposure_min_range, s_data->exposure_max_range);

	return err;
}

static int imx662_set_exposure_short(struct tegracam_device *tc_dev, s64 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx662 *priv = (struct imx662 *)tc_dev->priv;
	struct device *dev = tc_dev->dev;
	...
	integration_time_line = DIV_ROUND_CLOSEST(val
					* IMX662_K_FACTOR, priv->line_time);

	shr1 = IMX662_DEFAULT_RHS1 - integration_time_line;

	shr1 = shr1 - (shr1 % 2) + 1;

	if (shr1 < IMX662_MIN_SHR1_LENGTH)
		shr1 = IMX662_MIN_SHR1_LENGTH;
	else if (shr1 > IMX662_DEFAULT_RHS1 - IMX662_DOL2_MIN_INTEGRATION_LINES)
		shr1 = IMX662_DEFAULT_RHS1 - IMX662_DOL2_MIN_INTEGRATION_LINES;

	if (priv->broadcast_ctrl == BROADCAST)
		err = imx662_broadcast_buffered_reg(s_data, SHR1_LOW, 3, shr1);
	else
		err = imx662_write_buffered_reg(s_data, SHR1_LOW, 3, shr1);
...
static int imx662_update_exposure_ranges(struct tegracam_device *tc_dev)
{
	struct imx662 *priv = (struct imx662 *)tc_dev->priv;
	struct device *dev = tc_dev->dev;
	u8 min_shr0;
	...
	min_shr0 = hdr_en ? IMX662_DOL2_MIN_SHR0_LENGTH : IMX662_MIN_SHR0_LENGTH;
	...
	exposure_min_range = min_shr0 * priv->line_time / IMX662_K_FACTOR;
	if (hdr_en > 0) {
		exposure_max_range = (priv->frame_length - min_shr0) *
					priv->line_time / IMX662_K_FACTOR;
		short_exposure_min_range = (IMX662_DOL2_MIN_INTEGRATION_LINES *
					priv->line_time) / IMX662_K_FACTOR;
		short_exposure_max_range =
			((IMX662_DEFAULT_RHS1 - 10) * priv->line_time) /
							IMX662_K_FACTOR;
	} else {
	...
	if (hdr_en > 0) {
		fr_update_ctrl_range(tc_dev, TEGRA_CAMERA_CID_EXPOSURE_SHORT,
			short_exposure_min_range, short_exposure_max_range);
	}

	return ret;
}

static int imx662_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx662 *priv = (struct imx662 *)tc_dev->priv;
	struct device *dev = tc_dev->dev;
	...
	frame_length = (((u64)mode->control_properties.framerate_factor *
				IMX662_G_FACTOR) / (val * priv->line_time));
	...
	err = imx662_update_exposure_ranges(tc_dev);
	if (err < 0) {
...
static int imx662_set_test_pattern(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx662 *priv = (struct imx662 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	...
	if (val) {
		err = imx662_write_table(priv, mode_table[IMX662_EN_PATTERN_GEN]);
		if (err)
			goto fail;
		err = imx662_write_reg(s_data, TPG_PATSEL_DUOUT, (u8)(val - 1));
		if (err)
			goto fail;
	} else {
		err = imx662_write_table(priv, mode_table[IMX662_DIS_PATTERN_GEN]);
		if (err)
			goto fail;
	}
...
static int imx662_update_framerate_range(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx662 *priv = (struct imx662 *)tegracam_get_privdata(tc_dev);
	...
	if (hdr_en < 0) {
		dev_err(tc_dev->dev, "%s: unable to get hdr enable mode\n",
								__func__);
		return hdr_en;
	}
	...
	if (imx662_is_binning_mode(s_data))
		priv->min_frame_length = frame_height * 2
					+ IMX662_MIN_FRAME_LENGTH_DELTA;
	else
...
static int imx662_set_operation_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
...
static bool imx662_find_broadcast_sensor(struct imx662 *broadcast_private)
{
	struct imx662 *current_private;

	list_for_each_entry(current_private, &imx662_sensor_list, entry) {
...
static void imx662_enable_second_slave_address(struct imx662 *ack_private)
{
	struct imx662 *current_private;
	int err;
	...
	list_for_each_entry(current_private, &imx662_sensor_list, entry) {
		...
		err = imx662_write_reg(current_private->s_data,
							SECOND_SLAVE_ADD, 1);
		if (err)
	...
	err = imx662_write_reg(ack_private->s_data, SECOND_SLAVE_ADD, 3);
	if (err)
		dev_warn(&ack_private->i2c_client->dev,
			"%s: Fail to write Second I2C register\n", __func__);

	dev_dbg(&ack_private->i2c_client->dev,
		": Sensors 2nd slave address configured with acknowledge\n");

}

static void imx662_disable_second_slave_address(void)
{
	struct imx662 *current_private;
	int err;
...
static void imx662_configure_second_slave_address(void)
{
	struct imx662 broadcast_private = {};

	if (imx662_find_broadcast_sensor(&broadcast_private))
		imx662_enable_second_slave_address(&broadcast_private);
	else
		imx662_disable_second_slave_address();
}

static int imx662_set_broadcast_ctrl(struct tegracam_device *tc_dev,
						struct v4l2_ctrl *ctrl)
{
	struct imx662 *priv = (struct imx662 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	...
	imx662_configure_second_slave_address();

	return 0;
}

static int imx662_power_on(struct camera_common_data *s_data)
{
	int err = 0;
	struct camera_common_power_rail *pw = s_data->power;
	struct camera_common_pdata *pdata = s_data->pdata;
	struct device *dev = s_data->dev;
	struct imx662 *priv = (struct imx662 *)s_data->priv;
...
static int imx662_power_off(struct camera_common_data *s_data)
{
	struct camera_common_power_rail *pw = s_data->power;
	struct camera_common_pdata *pdata = s_data->pdata;
	struct device *dev = s_data->dev;
	...
	err = imx662_write_reg(s_data, XVS_DRV_XHS_DRV, 0xF);
	if (err)
...
static int imx662_power_get(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
	struct camera_common_data *s_data = tc_dev->s_data;
	struct camera_common_power_rail *pw = s_data->power;
...
static int imx662_power_put(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct camera_common_power_rail *pw = s_data->power;
	struct camera_common_pdata *pdata = s_data->pdata;
...
static int imx662_communication_verify(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx662 *priv = (struct imx662 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	...
	err = imx662_read_buffered_reg(s_data, VMAX_LOW, 2, &vmax);
	if (err) {
...
static struct camera_common_pdata *imx662_parse_dt(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
...
static int imx662_set_pixel_format(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
	struct imx662 *priv = (struct imx662 *)tegracam_get_privdata(tc_dev);
	struct camera_common_data *s_data = tc_dev->s_data;
	...
	case MEDIA_BUS_FMT_SRGGB10_1X10:
		err = imx662_write_table(priv, mode_table[IMX662_10BIT_MODE]);
		break;
	case MEDIA_BUS_FMT_SRGGB12_1X12:
		err = imx662_write_table(priv, mode_table[IMX662_12BIT_MODE]);
		break;
	default:
...
static int imx662_set_csi_lane_mode(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;

	if (s_data->numlanes == IMX662_TWO_LANE_MODE) {
		err = imx662_write_reg(s_data, LANEMODE, 1);
		if (err) {
...
static int imx662_calculate_line_time(struct tegracam_device *tc_dev)
{
	struct imx662 *priv = (struct imx662 *)tc_dev->priv;
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	...
	err = imx662_read_buffered_reg(s_data, HMAX_LOW, 2, &hmax);
	if (err) {
		dev_err(dev, "%s: unable to read hmax\n", __func__);
		return err;
	}

	priv->line_time = (hmax*IMX662_G_FACTOR) / (IMX662_INCK);
...
static int imx662_adjust_hmax_register(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;
	...
	switch (fr_get_v4l2_ctrl_value(tc_dev, TEGRA_CAMERA_CID_DATA_RATE,
								CURRENT)) {
	case IMX662_1440_MBPS:
		hmax = 660;
		break;
	case IMX662_1188_MBPS:
		if (s_data->mode == IMX662_MODE_1920x1080 ||
			s_data->mode == IMX662_MODE_DOL_HDR ||
			s_data->mode == IMX662_MODE_1280x720 ||
			s_data->mode == IMX662_MODE_640x480)
			hmax = 990;
		break;
	case IMX662_891_MBPS:
			if (imx662_is_binning_mode(s_data))
				hmax = 660;
			else
				hmax = 990;
		break;
	case IMX662_720_MBPS:
		if (s_data->numlanes == IMX662_TWO_LANE_MODE)
			hmax = 1980;
		else
			hmax = 660;
		break;
	case IMX662_594_MBPS:
		if (s_data->numlanes == IMX662_TWO_LANE_MODE) {
			if (imx662_is_binning_mode(s_data))
				hmax = 990;
			else
	...
	err = imx662_write_buffered_reg(s_data, HMAX_LOW, 2, hmax);
	if (err) {
...
static int imx662_verify_data_rate(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	struct v4l2_ctrl *ctrl;
	...
	if (s_data->numlanes == IMX662_TWO_LANE_MODE) {
		if (imx662_is_binning_mode(s_data)) {
			switch (fr_get_v4l2_ctrl_value(tc_dev,
				TEGRA_CAMERA_CID_DATA_RATE, CURRENT)) {
			case IMX662_720_MBPS:
			case IMX662_1188_MBPS:
			case IMX662_1440_MBPS:
				data_rate = IMX662_891_MBPS;
				goto modify_ctrl;
			}
		} else {
			switch (fr_get_v4l2_ctrl_value(tc_dev,
...
static int imx662_set_data_rate(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;
	...
	err = imx662_verify_data_rate(tc_dev);
	if (err)
		goto fail;

	if (s_data->power->state == SWITCH_ON) {
		err = imx662_write_reg(s_data, DATARATE_SEL,
				fr_get_v4l2_ctrl_value(tc_dev,
					TEGRA_CAMERA_CID_DATA_RATE, CURRENT) + 3);
		if (err)
			goto fail;
	}
...
static int imx662_set_sync_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err = 0;
	...
	if (s_data->power->state == SWITCH_ON) {
		err = imx662_write_reg(s_data, EXTMODE, extmode);
		if (err)
...
static int imx662_configure_triggering_pins(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err = 0;
	...
	err = imx662_write_reg(s_data, XVS_DRV_XHS_DRV, xvs_xhs_drv);
	if (err) {
 ...
 */
static int imx662_check_unsupported_mode(struct camera_common_data *s_data,
...
static int imx662_after_set_pixel_format(struct camera_common_data *s_data)
{
	struct device *dev = s_data->dev;
	...
	if (priv->current_pixel_format != s_data->colorfmt->code) {
		...
		switch (s_data->colorfmt->code) {
		case MEDIA_BUS_FMT_SRGGB10_1X10:
			err = imx662_update_ctrl(tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
					(*ctrl->p_cur.p_s64 >> 2),
					IMX662_DEFAULT_BLACK_LEVEL_10BPP,
					0, IMX662_MAX_BLACK_LEVEL_10BPP);
			priv->current_pixel_format = MEDIA_BUS_FMT_SRGGB10_1X10;
			break;
		case MEDIA_BUS_FMT_SRGGB12_1X12:
			err = imx662_update_ctrl(tc_dev,
						TEGRA_CAMERA_CID_BLACK_LEVEL,
						(*ctrl->p_cur.p_s64 << 2),
					IMX662_DEFAULT_BLACK_LEVEL_12BPP,
					0, IMX662_MAX_BLACK_LEVEL_12BPP);
...
static int imx662_set_mode(struct tegracam_device *tc_dev)
{
	struct imx662 *priv = (struct imx662 *)tegracam_get_privdata(tc_dev);
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	...
	err = imx662_write_table(priv, mode_table[IMX662_INIT_SETTINGS]);
	if (err) {
	...
	err = imx662_set_csi_lane_mode(tc_dev);
	if (err) {
		dev_err(dev, "%s: error setting CSI lane mode\n", __func__);
		return err;
	}

	err = imx662_set_pixel_format(tc_dev);
	if (err) {
	...
	err = imx662_write_table(priv, mode_table[s_data->mode]);
	if (err) {
		dev_err(dev, "%s: unable to write mode", __func__);
		return err;
	}

	err = imx662_set_operation_mode(tc_dev, fr_get_v4l2_ctrl_value(tc_dev,
	...
	err = imx662_set_sync_mode(tc_dev, fr_get_v4l2_ctrl_value(tc_dev,
	...
	err = imx662_configure_triggering_pins(tc_dev);
	if (err) {
		dev_err(dev, "%s: unable configure XVS/XHS pins\n", __func__);
		return err;
	}

	err = imx662_set_data_rate(tc_dev, fr_get_v4l2_ctrl_value(tc_dev,
					TEGRA_CAMERA_CID_DATA_RATE, CURRENT));
	if (err) {
		dev_err(dev, "%s: unable to set data rate\n", __func__);
		return err;
	...
	err = imx662_set_test_pattern(tc_dev, fr_get_v4l2_ctrl_value(tc_dev,
	...
	err = imx662_adjust_hmax_register(tc_dev);
	if (err) {
	...
	err = imx662_calculate_line_time(tc_dev);
	if (err)
		return err;

	err = imx662_update_framerate_range(tc_dev);
	if (err)
		return err;
...
static int imx662_start_streaming(struct tegracam_device *tc_dev)
{
	struct imx662 *priv = (struct imx662 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	struct camera_common_data *s_data = tc_dev->s_data;
	...
	err = imx662_write_table(priv, mode_table[IMX662_MODE_START_STREAM]);
	if (err)
		goto exit;
	...
	err = imx662_write_reg(s_data, XMSTA, xmsta);
	if (err)
		goto exit;
...
static int imx662_stop_streaming(struct tegracam_device *tc_dev)
{
	struct imx662 *priv = (struct imx662 *)tegracam_get_privdata(tc_dev);
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	...
	err = imx662_write_table(priv, mode_table[IMX662_MODE_STOP_STREAM]);
	if (err)
		return err;

	usleep_range(priv->frame_length * priv->line_time / IMX662_K_FACTOR,
		priv->frame_length * priv->line_time / IMX662_K_FACTOR + 1000);

	return 0;
}

static struct camera_common_sensor_ops imx662_common_ops = {
	.numfrmfmts = ARRAY_SIZE(imx662_frmfmt),
	.frmfmt_table = imx662_frmfmt,
	.power_on = imx662_power_on,
	.power_off = imx662_power_off,
	.write_reg = imx662_write_reg,
	.read_reg = imx662_read_reg,
	.parse_dt = imx662_parse_dt,
	.power_get = imx662_power_get,
	.power_put = imx662_power_put,
	.set_mode = imx662_set_mode,
	.start_streaming = imx662_start_streaming,
	.stop_streaming = imx662_stop_streaming,
	.check_unsupported_mode = imx662_check_unsupported_mode,
	.after_set_pixel_format = imx662_after_set_pixel_format,
};
...
static int imx662_board_setup(struct imx662 *priv)
{
	struct camera_common_data *s_data = priv->s_data;
	struct device *dev = s_data->dev;
	...
	if (!strcmp(priv->s_data->pdata->gmsl, "gmsl")) {
		...
		err = imx662_gmsl_serdes_setup(priv);
		if (err) {
	...
	err = imx662_power_on(s_data);
	if (err) {
		dev_err(dev, "Error %d during power on sensor\n", err);
		return err;
	}

	err = imx662_communication_verify(priv->tc_dev);
	if (err) {
	...
	err = imx662_calculate_line_time(priv->tc_dev);
	if (err) {
		dev_err(dev, "%s: unable to calculate line time\n", __func__);
		goto error2;
	}

	priv->min_frame_length = IMX662_DEFAULT_HEIGHT +
						IMX662_MIN_FRAME_LENGTH_DELTA;

error2:
	imx662_power_off(s_data);
...
static const struct v4l2_subdev_internal_ops imx662_subdev_internal_ops = {
	.open = imx662_open,
};

static struct tegracam_ctrl_ops imx662_ctrl_ops = {
	.numctrls = ARRAY_SIZE(ctrl_cid_list),
	.ctrl_cid_list = ctrl_cid_list,
	.set_gain = imx662_set_gain,
	.set_conversion_gain = imx662_set_conversion_gain,
	.set_exposure = imx662_set_exposure,
	.set_frame_rate = imx662_set_frame_rate,
	.set_group_hold = imx662_set_group_hold,
	.set_test_pattern = imx662_set_test_pattern,
	.set_data_rate = imx662_set_data_rate,
	.set_operation_mode = imx662_set_operation_mode,
	.set_sync_mode = imx662_set_sync_mode,
	.set_broadcast_ctrl = imx662_set_broadcast_ctrl,
	.set_black_level = imx662_set_black_level,
	.set_exposure_short = imx662_set_exposure_short,
};
...
#endif
{
	...
	priv = devm_kzalloc(dev,
			sizeof(struct imx662), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;
	...
	tc_dev->dev = dev;
	strncpy(tc_dev->name, "imx662", sizeof(tc_dev->name));
	tc_dev->dev_regmap_config = &sensor_regmap_config;
	tc_dev->sensor_ops = &imx662_common_ops;
	tc_dev->v4l2sd_internal_ops = &imx662_subdev_internal_ops;
	tc_dev->tcctrl_ops = &imx662_ctrl_ops;
	...
	err = imx662_update_ctrl(tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
			IMX662_DEFAULT_BLACK_LEVEL_10BPP,
					IMX662_DEFAULT_BLACK_LEVEL_10BPP, 0,
						IMX662_MAX_BLACK_LEVEL_10BPP);
	if (err)
		return err;

	err = imx662_update_ctrl(tc_dev, TEGRA_CAMERA_CID_TEST_PATTERN, 0, 0, 0,
				(ARRAY_SIZE(imx662_test_pattern_menu)-1));
	if (err)
		return err;

	err = imx662_update_ctrl(tc_dev, TEGRA_CAMERA_CID_DATA_RATE, 0, 0, 0,
					(ARRAY_SIZE(imx662_data_rate_menu)-1));
	if (err)
		return err;

	list_add_tail(&priv->entry, &imx662_sensor_list);
...
#endif
{
	...
	priv = (struct imx662 *)s_data->priv;

	if (!(strcmp(s_data->pdata->gmsl, "gmsl")))
		imx662_gmsl_serdes_reset(priv);
...
MODULE_DESCRIPTION("Media Controller driver for Sony IMX662");
