 ...
 *
 * fr_imx675.c - Framos fr_imx675.c driver
 */
...
#include "fr_imx675_mode_tbls.h"
#include "media/fr_sensor_common.h"

#define IMX675_K_FACTOR 1000LL
#define IMX675_M_FACTOR 1000000LL
#define IMX675_G_FACTOR 1000000000LL
#define IMX675_T_FACTOR 1000000000000LL

#define IMX675_MAX_GAIN_DEC 240
#define IMX675_MAX_GAIN_DB 72
#define IMX675_HIGH_GAIN_REG_MIN 4

#define IMX675_MAX_BLACK_LEVEL_10BPP 1023
#define IMX675_MAX_BLACK_LEVEL_12BPP 4095
#define IMX675_DEFAULT_BLACK_LEVEL_10BPP 50
#define IMX675_DEFAULT_BLACK_LEVEL_12BPP 200

#define IMX675_MIN_SHR0_LENGTH 4
#define IMX675_MIN_INTEGRATION_LINES 1

#define IMX675_4_CSI_LANES 4
#define IMX675_TWO_LANE_MODE 2

#define IMX675_INCK 74250000LL

LIST_HEAD(imx675_sensor_list);

static struct mutex serdes_lock__;

static const struct of_device_id imx675_of_match[] = {
	{ .compatible = "framos,imx675",},
	{ },
};
MODULE_DEVICE_TABLE(of, imx675_of_match);

const char * const imx675_data_rate_menu[] = {
	[IMX675_2376_MBPS] = "2376 Mbps/lane",
	[IMX675_2079_MBPS] = "2079 Mbps/lane",
	[IMX675_1782_MBPS] = "1782 Mbps/lane",
	[IMX675_1440_MBPS] = "1440 Mbps/lane",
	[IMX675_1188_MBPS] = "1188 Mbps/lane",
	[IMX675_891_MBPS] = "891 Mbps/lane",
	[IMX675_720_MBPS] = "720 Mbps/lane",
	[IMX675_594_MBPS] = "594 Mbps/lane",
};
...
static bool imx675_is_binning_mode(struct camera_common_data *s_data)
{
	switch (s_data->mode) {
	case IMX675_MODE_H2V2_BINNING:
		return true;
	default:
		return false;
	}
...
static inline int imx675_read_reg(struct camera_common_data *s_data,
				u16 addr, u8 *val)
{
	int err = 0;
	u32 reg_val = 0;
...
static int imx675_write_reg(struct camera_common_data *s_data,
				u16 addr, u8 val)
{
	int err;
...
static int imx675_write_reg_broadcast(struct camera_common_data *s_data,
							u16 addr, u8 val)
{
	int err;
	struct device *dev = s_data->dev;
...
static int imx675_read_buffered_reg(struct camera_common_data *s_data,
				u16 addr_low, u8 number_of_registers, u64 *val)
{
	...
	*val = 0;
	...
	if (!s_data->group_hold_active) {
		err = imx675_write_reg(s_data, REGHOLD, 0x01);
		if (err) {
	...
	for (i = 0; i < number_of_registers; i++) {
		err = imx675_read_reg(s_data, addr_low + i, &reg);
		*val += reg << (i * 8);
		if (err) {
	...
	if (!s_data->group_hold_active) {
		err = imx675_write_reg(s_data, REGHOLD, 0x00);
		if (err) {
...
static int imx675_write_buffered_reg(struct camera_common_data *s_data,
				u16 addr_low, u8 number_of_registers, u64 val)
{
	int err, i;
	struct device *dev = s_data->dev;
	...
	for (i = 0; i < number_of_registers; i++) {
		err = imx675_write_reg(s_data, addr_low + i, (u8)(val >> (i * 8)));
		if (err) {
...
static int imx675_broadcast_buffered_reg(struct camera_common_data *s_data,
				u16 addr_low, u8 number_of_registers, u32 val)
{
	...
	if (!s_data->group_hold_active) {
		err = imx675_write_reg_broadcast(s_data, REGHOLD, 0x01);
		if (err) {
			dev_err(dev, "%s: GRP_PARAM_HOLD error\n", __func__);
			return err;
		}
	...
	for (i = 0; i < number_of_registers; i++) {
		err = imx675_write_reg_broadcast(s_data, addr_low + i,
					(u8)(val >> (i * 8)));
		if (err) {
			dev_err(dev, "%s: BUFFERED register write error\n", __func__);
			return err;
	...
	if (!s_data->group_hold_active) {
		err = imx675_write_reg_broadcast(s_data, REGHOLD, 0x00);
		if (err) {
			dev_err(dev, "%s: GRP_PARAM_HOLD error\n", __func__);
			return err;
		}
...
static int imx675_write_table(struct imx675 *priv,
				const imx675_reg table[])
{
	...
	return regmap_util_write_table_8(s_data->regmap,
					 table,
					 NULL, 0,
					 IMX675_TABLE_WAIT_MS,
					 IMX675_TABLE_END);
}

static int imx675_set_group_hold(struct tegracam_device *tc_dev, bool val)
{
	...
	err = imx675_write_reg(s_data, REGHOLD, val);
	if (err) {
		dev_err(dev, "%s: GRP_PARAM_HOLD error\n", __func__);
		return err;
	}
...
static int imx675_update_ctrl(struct tegracam_device *tc_dev, int ctrl_id,
		u64 current_val, u64 default_val, u64 min_val, u64 max_val)
{
	struct imx675 *priv = (struct imx675 *)tegracam_get_privdata(tc_dev);
	...
	if (ctrl) {
		...
		case TEGRA_CAMERA_CID_TEST_PATTERN:
			ctrl->qmenu = imx675_test_pattern_menu;
			ctrl->maximum = max_val;
			break;
		case TEGRA_CAMERA_CID_DATA_RATE:
			ctrl->qmenu = imx675_data_rate_menu;
			ctrl->maximum = max_val;
			break;
		}
	}
...
static int imx675_set_black_level(struct tegracam_device *tc_dev, s64 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;
	...
	err = imx675_write_buffered_reg(s_data, BLKLEVEL_LOW, 2, black_level_reg);
	if (err) {
...
static int imx675_set_gain(struct tegracam_device *tc_dev, s64 val)
{
	...
	gain = val * IMX675_MAX_GAIN_DEC /
				 (IMX675_MAX_GAIN_DB *
					 mode->control_properties.gain_factor);

	if (priv->broadcast_ctrl == BROADCAST)
		err = imx675_broadcast_buffered_reg(s_data, GAIN_LOW, 2, gain);
	else
		err = imx675_write_buffered_reg(s_data, GAIN_LOW, 2, gain);
	if (err) {
...
static int imx675_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
	...
	integration_time_line = (val * IMX675_K_FACTOR) / priv->line_time;

	reg_shr0 = priv->frame_length - integration_time_line;

	reg_shr0 = (reg_shr0 % 2) ? reg_shr0 + 1 : reg_shr0;

	if (reg_shr0 < IMX675_MIN_SHR0_LENGTH)
		reg_shr0 = IMX675_MIN_SHR0_LENGTH;
	else if (reg_shr0 > (priv->frame_length - IMX675_MIN_INTEGRATION_LINES))
		reg_shr0 = priv->frame_length - IMX675_MIN_INTEGRATION_LINES;

	if (priv->broadcast_ctrl == BROADCAST)
		err = imx675_broadcast_buffered_reg(s_data, SHR0_LOW, 3, reg_shr0);
	else
		err = imx675_write_buffered_reg(s_data, SHR0_LOW, 3, reg_shr0);
	if (err) {
...
static int imx675_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
	...
	frame_length = (((u64)mode->control_properties.framerate_factor *
				IMX675_G_FACTOR) / (val * priv->line_time));
	...
	exposure_min_range = IMX675_MIN_INTEGRATION_LINES
				* priv->line_time / IMX675_K_FACTOR;

	exposure_max_range = (priv->frame_length - IMX675_MIN_INTEGRATION_LINES)
				* priv->line_time / IMX675_K_FACTOR;
	fr_update_ctrl_range(tc_dev, TEGRA_CAMERA_CID_EXPOSURE,
					exposure_min_range, exposure_max_range);

	if (priv->broadcast_ctrl == BROADCAST)
		err = imx675_broadcast_buffered_reg(s_data,
					VMAX_LOW, 3, priv->frame_length);
	else
		err = imx675_write_buffered_reg(s_data,
					VMAX_LOW, 3, priv->frame_length);
	if (err) {
		dev_err(dev, "%s: failed to set frame length\n", __func__);
		return err;
...
static int imx675_set_test_pattern(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx675 *priv = (struct imx675 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	...
	if (val) {
		err = imx675_write_table(priv, mode_table[IMX675_EN_PATTERN_GEN]);
		if (err)
			goto fail;
		err = imx675_write_reg(s_data, TPG_PATSEL_DUOUT, (u8)(val - 1));
		if (err)
			goto fail;
	} else {
		err = imx675_write_table(priv, mode_table[IMX675_DIS_PATTERN_GEN]);
		if (err)
			goto fail;
	}
...
static int imx675_update_framerate_range(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx675 *priv = (struct imx675 *)tegracam_get_privdata(tc_dev);
	...
	if (imx675_is_binning_mode(s_data))
		priv->min_frame_length = IMX675_DEFAULT_HEIGHT
						+ IMX675_MIN_FRAME_LENGTH_DELTA;
	else
		priv->min_frame_length = s_data->fmt_height
						+ IMX675_MIN_FRAME_LENGTH_DELTA;

	max_framerate = (IMX675_G_FACTOR * IMX675_M_FACTOR) /
...
static int imx675_set_operation_mode(struct tegracam_device *tc_dev, u32 val)
{
	return 0;
}

static bool imx675_find_broadcast_sensor(struct imx675 *broadcast_private)
{
	struct imx675 *current_private;

	list_for_each_entry(current_private, &imx675_sensor_list, entry) {
...
static void imx675_enable_second_slave_address(struct imx675 *ack_private)
{
	struct imx675 *current_private;
	int err;
	...
	list_for_each_entry(current_private, &imx675_sensor_list, entry) {
		...
		err = imx675_write_reg(current_private->s_data,
								SECOND_SLAVE_ADD, 1);
		if (err)
	...
	err = imx675_write_reg(ack_private->s_data,
								SECOND_SLAVE_ADD, 3);
	if (err)
...
static void imx675_disable_second_slave_address(void)
{
	struct imx675 *current_private;
	int err;
...
static void imx675_configure_second_slave_address(void)
{
	struct imx675 broadcast_private = {};

	if (imx675_find_broadcast_sensor(&broadcast_private))
		imx675_enable_second_slave_address(&broadcast_private);
	else
		imx675_disable_second_slave_address();
}

static int imx675_set_broadcast_ctrl(struct tegracam_device *tc_dev,
							struct v4l2_ctrl *ctrl)
{
	struct imx675 *priv = (struct imx675 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	...
	imx675_configure_second_slave_address();

	return 0;
}

static int imx675_power_on(struct camera_common_data *s_data)
{
	int err = 0;
	struct camera_common_power_rail *pw = s_data->power;
	struct camera_common_pdata *pdata = s_data->pdata;
	struct device *dev = s_data->dev;
	struct imx675 *priv = (struct imx675 *)s_data->priv;
...
static int imx675_power_off(struct camera_common_data *s_data)
{
	struct camera_common_power_rail *pw = s_data->power;
	struct camera_common_pdata *pdata = s_data->pdata;
	struct device *dev = s_data->dev;
	...
	err = imx675_write_reg(s_data, XVS_XHS_DRV, 0xF);
	if (err)
...
static int imx675_power_get(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
	struct camera_common_data *s_data = tc_dev->s_data;
	struct camera_common_power_rail *pw = s_data->power;
...
static int imx675_power_put(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct camera_common_power_rail *pw = s_data->power;
	struct camera_common_pdata *pdata = s_data->pdata;
...
static int imx675_communication_verify(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct imx675 *priv = (struct imx675 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	...
	err = imx675_read_buffered_reg(s_data, VMAX_LOW, 2, &vmax);
	if (err) {
...
static struct camera_common_pdata *imx675_parse_dt(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
...
static int imx675_set_pixel_format(struct tegracam_device *tc_dev)
{
	struct device *dev = tc_dev->dev;
	struct imx675 *priv = (struct imx675 *)tegracam_get_privdata(tc_dev);
	struct camera_common_data *s_data = tc_dev->s_data;
	...
	case MEDIA_BUS_FMT_SRGGB10_1X10:
		err = imx675_write_table(priv, mode_table[IMX675_10BIT_MODE]);
		break;
	case MEDIA_BUS_FMT_SRGGB12_1X12:
		err = imx675_write_table(priv, mode_table[IMX675_12BIT_MODE]);
		break;
	default:
...
static int imx675_set_csi_lane_mode(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;

	if (s_data->numlanes == IMX675_TWO_LANE_MODE) {
		err = imx675_write_reg(s_data, LANEMODE, 1);
		if (err) {
...
static int imx675_calculate_line_time(struct tegracam_device *tc_dev)
{
	struct imx675 *priv = (struct imx675 *)tc_dev->priv;
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	...
	err = imx675_read_buffered_reg(s_data, HMAX_LOW, 2, &hmax);
	if (err) {
		dev_err(dev, "%s: unable to read hmax\n", __func__);
		return err;
	}

	priv->line_time = (hmax*IMX675_G_FACTOR) / (IMX675_INCK);
...
static int imx675_adjust_hmax_register(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;
	...
	case IMX675_2376_MBPS:
		if (s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB10_1X10)
			hmax = 550;
		else
			hmax = 602;
		break;
	case IMX675_2079_MBPS:
		if (s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB10_1X10)
			hmax = 660;
		else
			hmax = 675;
		break;
	case IMX675_1440_MBPS:
		if (s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB10_1X10) {
	...
	case IMX675_1188_MBPS:
		...
		break;
	case IMX675_720_MBPS:
		if (s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB12_1X12)
			if (s_data->mode == IMX675_MODE_H2V2_BINNING)
				hmax = 452;
			else
				hmax = 1125;
		else
	...
	err = imx675_write_buffered_reg(s_data, HMAX_LOW, 2, hmax);
	if (err) {
...
static int imx675_verify_data_rate(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	...
	if (s_data->numlanes == IMX675_TWO_LANE_MODE) {
		switch (fr_get_v4l2_ctrl_value(tc_dev, TEGRA_CAMERA_CID_DATA_RATE, CURRENT)) {
		case IMX675_1782_MBPS:
		case IMX675_891_MBPS:
		case IMX675_594_MBPS:
			dev_warn(dev, "%s: Selected data rate is not supported with 2 CSI lane mode, switching to default!\n", __func__);
			data_rate = IMX675_2376_MBPS;
			goto modify_ctrl;
		case IMX675_720_MBPS:
			if (s_data->mode != IMX675_MODE_H2V2_BINNING) {
				dev_warn(dev, "%s: Selected data rate is not supported with binning mode, switching to default!\n", __func__);
				data_rate = IMX675_2376_MBPS;
				goto modify_ctrl;
			}
			break;
		...
		case IMX675_1782_MBPS:
		case IMX675_891_MBPS:
			dev_warn(dev, "%s: Selected data rate is not supported with 4 CSI lane mode switching to default!\n", __func__);
			if (s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB10_1X10)
				data_rate = IMX675_1440_MBPS;
			else
				data_rate = IMX675_1188_MBPS;
			goto modify_ctrl;
		case IMX675_1440_MBPS:
			if (s_data->colorfmt->code == MEDIA_BUS_FMT_SRGGB12_1X12) {
...
static int imx675_set_data_rate(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err;
	...
	err = imx675_verify_data_rate(tc_dev);
	if (err)
		goto fail;

	if (s_data->power->state == SWITCH_ON) {
		err = imx675_write_reg(s_data, DATARATE_SEL,
...
static int imx675_set_sync_mode(struct tegracam_device *tc_dev, u32 val)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err = 0;
	...
	if (s_data->power->state == SWITCH_ON) {
		err = imx675_write_reg(s_data, EXTMODE, extmode);
		if (err)
...
static int imx675_configure_triggering_pins(struct tegracam_device *tc_dev)
{
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	int err = 0;
	...
	err = imx675_write_reg(s_data, XVS_XHS_DRV, xvs_xhs_drv);
	if (err) {
 ...
 */
static int imx675_check_unsupported_mode(struct camera_common_data *s_data,
...
static int imx675_after_set_pixel_format(struct camera_common_data *s_data)
{
	struct device *dev = s_data->dev;
	...
	if (priv->current_pixel_format != s_data->colorfmt->code) {
		...
		switch (s_data->colorfmt->code) {
		case MEDIA_BUS_FMT_SRGGB10_1X10:
			err = imx675_update_ctrl(tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
				(*ctrl->p_cur.p_s64 >> 2), IMX675_DEFAULT_BLACK_LEVEL_10BPP,
				 0, IMX675_MAX_BLACK_LEVEL_10BPP);
			priv->current_pixel_format = MEDIA_BUS_FMT_SRGGB10_1X10;
			break;
		case MEDIA_BUS_FMT_SRGGB12_1X12:
			err = imx675_update_ctrl(tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
				(*ctrl->p_cur.p_s64 << 2), IMX675_DEFAULT_BLACK_LEVEL_12BPP,
				0, IMX675_MAX_BLACK_LEVEL_12BPP);
...
static int imx675_set_mode(struct tegracam_device *tc_dev)
{
	struct imx675 *priv = (struct imx675 *)tegracam_get_privdata(tc_dev);
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	...
	err = imx675_write_table(priv, mode_table[IMX675_INIT_SETTINGS]);
	if (err) {
		dev_err(dev, "%s: unable to initialize sensor settings\n", __func__);
		return err;
	}

	err = imx675_set_csi_lane_mode(tc_dev);
	if (err) {
		dev_err(dev, "%s: error setting CSI lane mode\n", __func__);
		return err;
	}

	err = imx675_set_pixel_format(tc_dev);
	if (err) {
		dev_err(dev, "%s: unable to write format to image sensor\n", __func__);
		return err;
	}

	err = imx675_write_table(priv, mode_table[s_data->mode]);
	if (err)
		return err;

	err = imx675_set_operation_mode(tc_dev, fr_get_v4l2_ctrl_value(tc_dev,
	...
	err = imx675_set_sync_mode(tc_dev, fr_get_v4l2_ctrl_value(tc_dev,
	...
	err = imx675_configure_triggering_pins(tc_dev);
	if (err) {
		dev_err(dev, "%s: unable configure XVS/XHS pins\n", __func__);
		return err;
	}

	err = imx675_set_data_rate(tc_dev, fr_get_v4l2_ctrl_value(tc_dev,
	...
	err = imx675_set_test_pattern(tc_dev, fr_get_v4l2_ctrl_value(tc_dev,
	...
	err = imx675_adjust_hmax_register(tc_dev);
	if (err) {
	...
	err = imx675_calculate_line_time(tc_dev);
	if (err)
		return err;

	err = imx675_update_framerate_range(tc_dev);
	if (err)
		return err;
...
static int imx675_start_streaming(struct tegracam_device *tc_dev)
{
	struct imx675 *priv = (struct imx675 *)tegracam_get_privdata(tc_dev);
	struct device *dev = tc_dev->dev;
	struct camera_common_data *s_data = tc_dev->s_data;
	...
	err = imx675_write_table(priv, mode_table[IMX675_MODE_START_STREAM]);
	if (err)
		goto exit;
	...
	err = imx675_write_reg(s_data, XMSTA, xmsta);
	if (err)
		goto exit;
...
static int imx675_stop_streaming(struct tegracam_device *tc_dev)
{
	struct imx675 *priv = (struct imx675 *)tegracam_get_privdata(tc_dev);
	struct camera_common_data *s_data = tc_dev->s_data;
	struct device *dev = tc_dev->dev;
	...
	err = imx675_write_table(priv, mode_table[IMX675_MODE_STOP_STREAM]);
	if (err)
		return err;

	usleep_range(priv->frame_length * priv->line_time / IMX675_K_FACTOR,
		priv->frame_length * priv->line_time / IMX675_K_FACTOR + 1000);
	return 0;
}

static struct camera_common_sensor_ops imx675_common_ops = {
	.numfrmfmts = ARRAY_SIZE(imx675_frmfmt),
	.frmfmt_table = imx675_frmfmt,
	.power_on = imx675_power_on,
	.power_off = imx675_power_off,
	.write_reg = imx675_write_reg,
	.read_reg = imx675_read_reg,
	.parse_dt = imx675_parse_dt,
	.power_get = imx675_power_get,
	.power_put = imx675_power_put,
	.set_mode = imx675_set_mode,
	.start_streaming = imx675_start_streaming,
	.stop_streaming = imx675_stop_streaming,
	.check_unsupported_mode = imx675_check_unsupported_mode,
	.after_set_pixel_format = imx675_after_set_pixel_format,
};

static int imx675_gmsl_serdes_setup(struct imx675 *priv)
{
	int err = 0;
	int des_err = 0;
	struct device *dev;
...
static void imx675_gmsl_serdes_reset(struct imx675 *priv)
{
	mutex_lock(&serdes_lock__);
...
static int imx675_board_setup(struct imx675 *priv)
{
	struct camera_common_data *s_data = priv->s_data;
	struct device *dev = s_data->dev;
	...
	if (!(strcmp(s_data->pdata->gmsl, "gmsl"))) {
		...
		err = imx675_gmsl_serdes_setup(priv);
		if (err) {
	...
	err = imx675_communication_verify(priv->tc_dev);
	if (err) {
		dev_err(dev, "%s: unable to communicate with sensor\n", __func__);
		goto error2;
	}

	err = imx675_calculate_line_time(priv->tc_dev);
	if (err) {
...
error2:
	imx675_power_off(s_data);
	camera_common_mclk_disable(s_data);

	return err;
}

static int imx675_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
...
static const struct v4l2_subdev_internal_ops imx675_subdev_internal_ops = {
	.open = imx675_open,
};

static struct tegracam_ctrl_ops imx675_ctrl_ops = {
	...
	.set_test_pattern = imx675_set_test_pattern,
	.set_data_rate = imx675_set_data_rate,
	.set_operation_mode = imx675_set_operation_mode,
	.set_sync_mode = imx675_set_sync_mode,
	.set_broadcast_ctrl = imx675_set_broadcast_ctrl,
	.set_black_level = imx675_set_black_level,
};

#if defined(NV_I2C_DRIVER_STRUCT_PROBE_WITHOUT_I2C_DEVICE_ID_ARG) /* Linux 6.3 */
static int imx675_probe(struct i2c_client *client)
#else
static int imx675_probe(struct i2c_client *client, const struct i2c_device_id *id)
#endif
{
	...
	priv = devm_kzalloc(dev,
			sizeof(struct imx675), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;
	...
	tc_dev->dev = dev;
	strncpy(tc_dev->name, "imx675", sizeof(tc_dev->name));
	tc_dev->dev_regmap_config = &sensor_regmap_config;
	tc_dev->sensor_ops = &imx675_common_ops;
	tc_dev->v4l2sd_internal_ops = &imx675_subdev_internal_ops;
	tc_dev->tcctrl_ops = &imx675_ctrl_ops;
	...
	err = imx675_update_ctrl(tc_dev, TEGRA_CAMERA_CID_BLACK_LEVEL,
				IMX675_DEFAULT_BLACK_LEVEL_12BPP,
				IMX675_DEFAULT_BLACK_LEVEL_12BPP, 0,
				IMX675_MAX_BLACK_LEVEL_12BPP);
	if (err)
		return err;

	err = imx675_update_ctrl(tc_dev, TEGRA_CAMERA_CID_TEST_PATTERN,
			0, 0, 0, (ARRAY_SIZE(imx675_test_pattern_menu)-1));
	if (err)
		return err;

	err = imx675_update_ctrl(tc_dev, TEGRA_CAMERA_CID_DATA_RATE,
			0, 0, 0, (ARRAY_SIZE(imx675_data_rate_menu)-1));
	if (err)
		return err;

	list_add_tail(&priv->entry, &imx675_sensor_list);

	dev_info(dev, "Detected imx675 sensor\n");
...
#endif
{
	...
	priv = (struct imx675 *)s_data->priv;

	if (!(strcmp(s_data->pdata->gmsl, "gmsl")))
		imx675_gmsl_serdes_reset(priv);
