...
class MainMenu(object):
    ...
    def _get_configurations(self):
        ...
        if "Jetson Thor" in self.jetson.model:
            self.jetson.set_active_header("Jetson AGX CSI Connector")
            self.current_board = "Jetson AGX Thor"
        elif "Jetson AGX CSI Connector" in self.jetson.board_headers:
            self.jetson.set_active_header("Jetson AGX CSI Connector")
    ...
    def _update_extlinux_file(self):
        ...
        elif self.current_board == "Jetson AGX Thor":
            self.board_dtbo_template = "tegra264-p3971-camera-fr_"
        else:
    ...
    def _update_fpa_configuration(self):
        ...
        updateCmd = "sed -i 's/OVERLAYS.*/OVERLAYS \\/boot\\/framos\\/dtbo\\/"
        updateCmd += fpaDTBO + "/' "
        updateCmd += self.extLinuxFile
        subprocess.run([updateCmd], shell=True)
...
class MainWindow(jetsonio.MainWindow):
    def __init__(self, screen, h, w):
        title = " Jetson Config Camera v2.1.0 "
